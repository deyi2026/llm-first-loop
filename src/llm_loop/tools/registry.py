...
_COMPACT_TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "browser_semantic_operation": "Browser语义执行：steps仅是调用容器，单个已决定动作直接调用；多个动作仅在模型已经决定时按序提交。target=exact kind+name(+role)；wait=until状态，wait_text=文本匹配；程序只做ground/version/poll/single-dispatch；不fuzzy/rebind/retry/完成判断。",
