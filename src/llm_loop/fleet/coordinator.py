...
class ProjectCoordinator:
    def __init__(
        ...
        *,
        ...
        try:
            ...
            self.store.create_project(project_id)
    ...
    def _workspace_for(self, run_key: str) -> ExecutionWorkspace:
        ...
        for workspace_id in self.store.get_project(self.project_id).workspaces:
            workspace = self.store.get_workspace(workspace_id)
            if workspace.project_id == self.project_id and workspace.physical_root == run_root:
                return workspace
        return self.store.create_workspace(self.project_id, run_root, self.repo_head)

    # -------------------------------------------------------- run boundary

    def begin_run(self, run_key: str, *, worker_id: str | None = None) -> WorkerLease:
        ...
        return self.store.acquire_lease(
            self.project_id, workspace.workspace_id, self.owner_id, worker_id=worker_id
        )
    ...
    def reclaim_run(self, workspace_id: str) -> WorkerLease:
        return self.store.reclaim(self.project_id, workspace_id, self.owner_id)
