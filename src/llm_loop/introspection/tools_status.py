...
def run_status(ctx: Any, status_provider: Any, args: dict) -> ToolResult:
    ...
    if scope == "runtime":
        ...
        if supports_session_scope:
            ...
            sensitive = {"action_trace", "tool_history", "message_flow", "exception_log", "causality"}
            if sensitive.intersection(dims or ()):
                return ToolResult(
                    status=ToolResultStatus.FAILURE,
                    content=(
                        "[架构状态隔离拒绝] 当前 status provider 不支持 session-scoped snapshot；"
                        "为避免跨会话或无归因运行时事实混入，拒绝返回会话敏感维度。"
                    ),
                    tool_call_id="",
                    tool_name="architecture_status",
                )
            # Legacy/testing providers may retain dimensions-only behavior for aggregate,
            # non-session-sensitive facts. Global operational inspection remains available
            # through explicit scope=runtime.
