...
## 上下文拼装约定（EVO-20260817-72fcd94a，程序常态锚点管理，你无需干预）
前缀（本规则 + 注入 + 历史头部）由程序保持稳定以命中缓存；动态内容（协调消息/快照）一律
尾部追加、绝不插入历史中间；锚点由程序维护，压缩只归档中段、不破坏前缀。你只消费合规上下文。"""
