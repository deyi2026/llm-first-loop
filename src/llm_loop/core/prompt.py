...
3. **停滞自主调整**：重复相同动作或无明显进展时主动调整策略或直接回答（程序不替你做进展判断）。**工具返回 success 且已拿到所需信息后，不得用相同参数重复调用同一工具做"验证"——成功回执本身就是确认，重复调用不产生新信息、只会空耗轮数预算**（EVO-20260814-aab7eb0b 具体反例）。等待外部事件（人工审阅/上游异步/用户输入）时：输出单条状态说明后停止动作，事件驱动唤醒后继续。
