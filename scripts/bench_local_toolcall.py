...
TOOLS = [
    {"type": "function", "function": {"name": "web_search", "description": "搜索网络获取最新信息",
    ...
    {"type": "function", "function": {"name": "set_reminder", "description": "设置定时提醒",
...
TASKS = [
    {"id": "T1", "user": "帮我搜索最新的AI行业新闻", "expect_tool": "web_search", "expect_params": {"query": "AI"}},
    {"id": "T2", "user": "计算一下 17 乘以 23 等于多少", "expect_tool": "calculator", "expect_params": {"expression": "17"}},
    {"id": "T3", "user": "5分钟后提醒我去开周会", "expect_tool": "set_reminder", "expect_params": {"message": "周会", "seconds": "300"}},
    {"id": "T4", "user": "帮我查一下北京明天的天气怎么样", "expect_tool": "get_weather", "expect_params": {"city": "北京"}},
    {"id": "T5", "user": "搜索一下 2025 年大模型落地应用案例，顺便看看杭州和深圳的天气", "expect_tool": "web_search", "expect_params": {"query": "大模型"}},
]

# 无关上下文填充（模拟长历史/大信息量）
LARGE_CTX = ("项目背景：本系统为 LLM-first 架构的自主运行平台，核心原则是程序作为感官和手脚、模型作为大脑。"
    "系统包含工具调用循环、记忆注入、经验沉淀、缓存优化、演进建议等多个模块。当前正在进行本地大模型工具调用性能优化专项。"
    "历史会话摘要：此前验证了前缀稳定对缓存命中的影响，确认 system prompt 中的动态内容会破坏前缀缓存。"
    "这是一段与当前请求无关的历史对话记录，用于测试大上下文对工具调用格式遵循的影响。") * 100
...
def check(msg, task):
    ...
    if not args_ok:
        return (False, tool_ok, False, f"参数JSON解析失败: {tc.get('function',{}).get('arguments','')[:100]!r}")
...
def run_case(model, task, prefix_mode, ctx_mode):
    ts = "2026-08-21 12:00:00" if prefix_mode == "static" else datetime.now().strftime("%Y-%m-%d %H:%M:%S.%f")
    sys_prompt = (f"你是工具调度助手。当前时间: {ts}。根据用户请求选择并调用最合适的工具。"
                  "只输出工具调用，不输出额外解释。")
    if ctx_mode == "large":
    ...
    try:
        ...
        return {"fmt": fmt, "tool": tool_ok, "param": param_ok, "dt": dt,
                "tokens": usage.get("total_tokens", 0), "note": note}
    except Exception as e:
        return {"fmt": False, "tool": False, "param": False, "dt": None,
                "tokens": None, "note": f"调用异常: {type(e).__name__}: {e}"}
        ...
        for pm in ("static", "dynamic"):
            for cm in ("small", "large"):
                ...
                print(f"{task['id']} [{pm}/{cm}] fmt={r['fmt']} tool={r['tool']} param={r['param']} "
                      f"dt={r['dt']:.1f}s tok={r['tokens']} | {r['note'][:80]}", flush=True)
    # 汇总
    print("\n== 汇总 ==")
    for pm in ("static", "dynamic"):
        for cm in ("small", "large"):
            ...
            print(f"[{pm}/{cm}] n={n} 格式成功率={f}/{n} 工具正确={t}/{n} 参数正确={p}/{n} "
                  f"平均耗时={sum(dts)/len(dts):.1f}s" if dts else f"[{pm}/{cm}] n={n} 格式成功率={f}/{n} 无耗时数据")
    # 前缀缓存观察：static 组合第2次起是否更快（同前缀重复）
    print("\n== 静态前缀耗时序列（观察前缀缓存）==")
    for task in TASKS:
        s = [r for r in results if r["task"] == task["id"] and r["prefix"] == "static" and r["ctx"] == "small"]
