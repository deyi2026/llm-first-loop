...
def sanitize_message(msg: dict) -> dict:
    ...
    if isinstance(c, str):
        ...
        out["content"] = [
            ...
            if isinstance(part.get("text"), str) else part
            for part in c
        ]
    ...
    if isinstance(params, dict):
        ...
        if isinstance(props, dict) and len(props) > TOOL_SCHEMA_LIMIT:
            ...
            params["properties"] = {
                ...
                if isinstance(v, dict) and isinstance(v.get("description"), str) else v
                for k, v in keep.items()
            }
            params["properties"]["__truncated__"] = f"[{len(props) - TOOL_SCHEMA_LIMIT} more omitted]"
    return out
...
def main() -> int:
    ...
    for cat, spec in CATEGORIES.items():
        try:
            ...
            print(f"[ok] {cat}: run={fx['meta']['tail_user_run']} msgs={fx['meta']['msg_count']} -> {out.name}")
