...
TOOLS = [
    ...
    },
    {
        "function": {
            "name": "translate_text",
            "description": "将文本翻译为目标语言。必填参数: text, target_lang",
            "parameters": {
                "type": "object",
                "properties": {
                    "text": {"type": "string", "description": "待翻译的原文内容"},
    ...
    },
    {
        "function": {
            "name": "summarize",
            "description": "对长文本生成摘要。必填参数: text",
            "parameters": {
                "type": "object",
                "properties": {
                    "text": {"type": "string", "description": "待摘要的长文本"},
                    "max_words": {"type": "integer", "description": "摘要最大字数（可选）"},
                },
                "required": ["text"],
            },
        }
    },
    {
        "function": {
            ...
            "description": "查询股票当前价格。必填参数: symbol",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        "function": {
            "name": "create_note",
            "description": "创建笔记并添加标签。必填参数: title, content",
            "parameters": {
                "type": "object",
                "properties": {
    ...
    },
    {
        "function": {
            "name": "send_email",
            "description": "发送邮件，支持定时发送。必填参数: to, subject, body；若指定定时发送则 schedule 必填",
            "parameters": {
                "type": "object",
                "properties": {
                    "to": {"type": "string"},
...
TASKS = [
    ...
    {
        "id": "T4",
        "user": "给 zhang@example.com 发一封邮件，主题是季度汇报，正文写 见附件，安排在明天上午10点发送",
    ...
    {
        "id": "T5",
        "user": "搜索最新的AI芯片新闻，顺便查一下深圳的天气",
        "expect_tools": ["web_search", "get_weather"],
        "expect_params": {"query": "芯片", "city": "深圳"},
    },
    {
        "id": "T6",
        "user": "把这段话翻译成英文：今天天气很好，我们出去散步吧",
        "expect_tools": ["translate_text"],
        "expect_params": {"text": "散步", "target_lang": "en"},
    },
]
...
def _hist_tail(n=8):
    ...
    for i in range(n):
        pairs += [
            ...
            '[助手] [调用工具 read_file 参数 {"path": "src/llm_loop/llm/client.py"}]',
...
def tools_text(tools):
    ...
    lines.append(
        '需要调用工具时，仅输出一行 JSON: {"tool": "工具名", "args": {...}}；多个调用用换行分隔，不要输出其他内容。'
    )
    return "\n".join(lines)
...
def call(model, content, timeout=300):
    ...
    try:
        conn.request(
            "POST", "/api/v1/chat", body=body, headers={"Content-Type": "application/json"}
        )
        ...
        for req_key in schema_required:
            ...
            for c in calls:
                ...
                if isinstance(v, dict) and v:  # 嵌套对象非空即存在（如 schedule 整体）
                    ...
                    break
            if not found:
                miss.append(f"required.{exp_tool}.{req_key}")
    # 值内容检查（expect_params 语义验证）
    for k, kw in task["expect_params"].items():
        found = False
        for c in calls:
            if c["name"] not in task["expect_tools"]:
                continue
            v = c["arguments"]
            for kk in k.split("."):
                if isinstance(v, dict) and kk in v:
...
def main():
    ...
    args = ap.parse_args()
    print(
        f"== v2 加严基准 模型={args.model} N={len(TASKS)} 组合=2x2 (真实lms-chat协议) ==",
        flush=True,
    )
    results = []
    for task in TASKS:
            ...
            for cm in ("small", "large"):
                ...
                try:
                    ...
                    r = {
                        ...
                        "prefill": stage(events, "_PREFILL_END"),
                        "reason": stage(events, "_REASON_END"),
                ...
                print(
                    ...
                    f"prefill={_fmt(r['prefill'])}s reason={_fmt(r['reason'])}s first_msg={_fmt(r['first_msg'])}s",
                    flush=True,
                )
                if not calls:
    ...
    for pm in ("static", "dynamic"):
        for cm in ("small", "large"):
            ...
            print(
                f"[{pm}/{cm}] n={n} 格式={f}/{n} 工具={t}/{n} 参数={p}/{n} "
