...
def build_styles():
    s = {}
    s['h1'] = ParagraphStyle('h1', fontName='STSong-Light', fontSize=17, leading=24,
                             textColor=C_PRIMARY, spaceBefore=4, spaceAfter=10)
    s['h2'] = ParagraphStyle('h2', fontName='STSong-Light', fontSize=13.5, leading=19,
                             textColor=C_PRIMARY, spaceBefore=10, spaceAfter=6)
    s['h3'] = ParagraphStyle('h3', fontName='STSong-Light', fontSize=11.5, leading=16,
                             textColor=C_ACCENT, spaceBefore=8, spaceAfter=4)
    s['body'] = ParagraphStyle('body', fontName='STSong-Light', fontSize=10, leading=15.5,
                               textColor=colors.HexColor('#222222'), spaceAfter=4)
    s['quote'] = ParagraphStyle('quote', parent=s['body'], leftIndent=10, fontSize=9.5,
                                leading=14.5, textColor=C_GREY)
    s['bullet'] = ParagraphStyle('bullet', parent=s['body'], leftIndent=12, bulletIndent=2)
    s['caption'] = ParagraphStyle('caption', fontName='STSong-Light', fontSize=9, leading=13,
                                  textColor=C_GREY, alignment=1, spaceBefore=4, spaceAfter=8)
    return s
...
if __name__ == '__main__':
    ...
    args = ap.parse_args()
    md2pdf(args.input, args.output, no_images=args.no_images, title=args.title,
