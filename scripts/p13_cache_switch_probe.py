...
def main():
    ...
    sys_prompt = "你是 llm-first-loop 缓存实测助手。" + "跨模型缓存共享探测前缀填充。" * 300  # ~2.4K tok
    ...
    def row(name, r):
        ti = r.prompt_tokens or 0; hit = r.prompt_cache_hit_tokens or 0
    ...
    if h3 > 0:
        print(f"✅ 跨模型共享成立: pro 命中 flash 前缀 {h3} tokens → 切换成本低于预期，可优化锚点策略")
    else:
