...
def slim_session(path, keep=50, dry_run=False):
    ...
    for line in lines:
        ...
        if t in ("tool.result", "tool_result", "tool.output", "tool_output") and kept < keep:
            ...
            slim["payload"] = {
                "slimmed": True,
                "tool_name": d.get("payload", {}).get("tool_name", ""),
    ...
    if orphans:
        ...
        return False
    old_chars = sum(len(line) for line in lines)
    new_chars = sum(len(line) for line in out)
    print(
        f"消息数: {total} → {len(out)} | 字符: {old_chars:,} → {new_chars:,} ({(1 - new_chars / old_chars) * 100:.0f}%) | 0 孤儿 ✓"
    )
    if dry_run:
        return True
    # 备份 + flock 写回
