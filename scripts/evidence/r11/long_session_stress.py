...
R11_ROOT = ROOT / "data/audit/evidence_r11"
SOURCE_DIR = R11_ROOT / "sources"
FROZEN_PATH = ROOT / "tests/fixtures/evidence_r11/frozen_v1.json"
...
def verify_frozen_pack() -> dict[str, Any]:
    if not FROZEN_PATH.exists():
        raise RuntimeError(f"R11 frozen lock missing: {FROZEN_PATH}")
    payload = json.loads(FROZEN_PATH.read_text(encoding="utf-8"))
    if payload.get("schema") != "evidence-r11-frozen-v1":
        raise RuntimeError("R11 frozen lock schema mismatch")
    mismatches: list[str] = []
    ...
    if mismatches:
        raise RuntimeError("R11 frozen artifact mismatch: " + "; ".join(mismatches))
    current_sources = prepare_sources()
    if current_sources != payload.get("source_hashes"):
        raise RuntimeError("R11 source fixture hashes mismatch")
    return payload


def _source_rel(index: int) -> str:
    return f"data/audit/evidence_r11/sources/r11_source_{index:02d}.txt"
...
def prepare_sources() -> dict[str, str]:
    ...
    for i, code in enumerate(CODES, 1):
        rows = [
            f"R11 file {i:02d} row {j:04d}: neutral long-session cache/compression material segment {j % 17:02d}."
            for j in range(360)
        ]
        rows.insert(177, f"R11-CODE-{i:02d}: {code}")
        text = "\n".join(rows) + "\n"
...
def scenario() -> list[dict[str, Any]]:
    ...
    return [
        ...
        {
            "turn": 3,
            "message": new(3) + " 同时给出第1轮文件的 R11-CODE-01 值。",
            "expect": [CODES[2], CODES[0]],
        },
        {"turn": 4, "message": new(4), "expect": [CODES[3]]},
        {
            "turn": 5,
            "message": new(5) + " 同时给出第2轮文件的 R11-CODE-02 值。",
            "expect": [CODES[4], CODES[1]],
        },
        {
            "turn": 6,
            "message": new(6) + " 同时给出第1轮文件的 R11-CODE-01 值。",
            "expect": [CODES[5], CODES[0]],
        },
        {"turn": 7, "message": new(7), "expect": [CODES[6]]},
        {
            "turn": 8,
            "message": new(8) + " 同时给出第3轮文件的 R11-CODE-03 值。",
            "expect": [CODES[7], CODES[2]],
        },
        {
            "turn": 9,
            "message": new(9) + " 同时给出第1轮文件的 R11-CODE-01 值。",
            "expect": [CODES[8], CODES[0]],
        },
        {
            "turn": 10,
            "message": new(10) + " 同时给出第5轮文件的 R11-CODE-05 值。",
            "expect": [CODES[9], CODES[4]],
        },
        {
            "turn": 11,
            "message": "请给出此前第1、4、7、10个文件各自的 R11-CODE 值；需要工具时自行决定。",
            "expect": [CODES[0], CODES[3], CODES[6], CODES[9]],
        },
        {
            "turn": 12,
            "message": "最终核对：请按 01→10 顺序列出本会话十个 R11-CODE 的值。",
            "expect": list(CODES),
        },
    ]
...
def start_server(mode: str, port: int) -> tuple[subprocess.Popen[bytes], Any]:
    ...
    log_path = R11_ROOT / f"web_{mode}.log"
    ...
    for _ in range(60):
        if proc.poll() is not None:
            log.flush()
            raise RuntimeError(f"R11 {mode} web exited early rc={proc.returncode}; see {log_path}")
        try:
            ...
            pass
        time.sleep(0.5)
    raise RuntimeError(f"R11 {mode} web did not become healthy; see {log_path}")
...
def analyze(mode: str, session_id: str, turn_results: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "tool_protocol_errors": tool_protocol_errors,
...
def per_mode_gates(report: dict[str, Any]) -> dict[str, bool]:
    ...
    return {
        ...
        "no_tool_protocol_error": report["tool_protocol_errors"] == 0,
        ...
        "no_unexplained_boundary_backjump": report["unexplained_cache_boundary_backjumps"] == 0,
...
def run_mode(mode: str, port: int = PORT) -> dict[str, Any]:
    ...
    try:
        warm = _post_chat(port, None, "R11 warmup：只回答 WARMUP_OK。", new=True)
        ...
        out = R11_ROOT / f"report_{mode}_v1.json"
...
def main() -> int:
    ...
    (R11_ROOT / "source_hashes_v1.json").write_text(
    ...
    if args.score:
        off = json.loads((R11_ROOT / "report_off_v1.json").read_text())
        enforce = json.loads((R11_ROOT / "report_enforce_v1.json").read_text())
        paired = paired_gates(off, enforce)
        payload = {
            "schema": "evidence-r11-score-v1",
            "status": "PASS"
        ...
        (R11_ROOT / "score_v1.json").write_text(
