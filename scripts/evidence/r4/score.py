...
def score_runs(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    by_seed = {seed: [r for r in completed if r.get("seed_id") == seed] for seed in ("K1", "K2", "K3", "K4", "K5", "K6")}
    ...
    provider_exact = {provider: _metric(by_provider[provider], lambda r: bool(r.get("final_answer_exact"))) for provider in PROVIDERS}
    source_once_rows = [r for seed in ("K1", "K2", "K3", "K5", "K6") for r in by_seed[seed]]
    source_once = _metric(source_once_rows, lambda r: int(r.get("source_execution_count") or 0) == 1)
    k1_exact = {provider: _metric([r for r in by_seed["K1"] if r.get("provider") == provider], lambda r: bool(r.get("final_answer_exact"))) for provider in PROVIDERS}
    k2_exact = {provider: _metric([r for r in by_seed["K2"] if r.get("provider") == provider], lambda r: bool(r.get("final_answer_exact"))) for provider in PROVIDERS}
    k4 = by_seed["K4"]
    k5 = by_seed["K5"]
    k6 = by_seed["K6"]
    ...
    gates = {
        ...
        "zero_transport_ref_as_domain_answer": all(not bool(r.get("transport_ref_as_domain_answer")) for r in completed),
        ...
        "k1_read_recovery_and_exact_each_provider_ge_2_of_3": all(
            k1_exact[p]["pass"] >= 2
            and all(
                (not r.get("final_answer_exact")) or int(r.get("read_evidence_success_count") or 0) >= 1
                for r in by_seed["K1"] if r.get("provider") == p
            )
            for p in PROVIDERS
        ),
        "k2_exact_each_provider_ge_2_of_3": all(k2_exact[p]["pass"] >= 2 for p in PROVIDERS),
        "k2_search_used_ge_4_of_6": sum(int(r.get("search_evidence_success_count") or 0) >= 1 for r in by_seed["K2"]) >= 4,
        "k6_exact_ge_5_of_6": sum(bool(r.get("final_answer_exact")) for r in k6) >= 5,
        "k6_recovery_used_and_not_stale_blocked": all(
            int(r.get("recovery_success_count") or 0) >= 1 and int(r.get("stale_block_count") or 0) == 0
            for r in k6
        ),
        "k3_zero_side_effect_duplicate": all(int(r.get("side_effect_duplicate_count") or 0) == 0 for r in by_seed["K3"]),
        ...
        "k4_current_exact_6_of_6": len(k4) == 6 and all(bool(r.get("final_answer_exact")) for r in k4),
        "k4_exactly_two_source_acquisitions": len(k4) == 6 and all(int(r.get("source_execution_count") or 0) == 2 for r in k4),
        "k5_source_once_all": len(k5) == 6 and all(int(r.get("source_execution_count") or 0) == 1 for r in k5),
    ...
    return {
        ...
        "metrics": {
            ...
            "k2_search_used": sum(int(r.get("search_evidence_success_count") or 0) >= 1 for r in by_seed["K2"]),
            "k3_exact": sum(bool(r.get("final_answer_exact")) for r in by_seed["K3"]),
            "k3_side_effect_duplicates": sum(int(r.get("side_effect_duplicate_count") or 0) for r in by_seed["K3"]),
            ...
            "k5_historical_access": sum(int(r.get("historical_access_success_count") or 0) >= 1 for r in k5),
