...
DEFAULT_INPUT = ROOT / "data/audit/evidence_r7v2/runs_real_v1.json"
DEFAULT_OUTPUT = ROOT / "data/audit/evidence_r7v2/score_real_v1.json"
...
def score_runs(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    groups = {
        seed: [r for r in rows if r.get("seed_id") == seed] for seed in ("Q1", "Q2", "Q3", "Q4")
    }
    covered = groups["Q1"] + groups["Q2"]
    covered_by_provider = {
        p: _rate(
    ...
    q3_by_provider = {
        p: _rate(
            [r for r in groups["Q3"] if r.get("provider") == p],
            lambda r: bool(r.get("final_answer_exact")),
        )
        for p in PROVIDERS
    }
    q4_by_provider = {
        p: _rate(
            [r for r in groups["Q4"] if r.get("provider") == p],
            lambda r: bool(r.get("final_answer_exact")),
        )
        for p in PROVIDERS
    }
    ...
    q3_exact = [r for r in groups["Q3"] if r.get("final_answer_exact")]
    q4_exact = [r for r in groups["Q4"] if r.get("final_answer_exact")]
    ...
    gates = {
        ...
        == 0,
        "q3_stale_as_current_zero": sum(bool(r.get("stale_as_current")) for r in groups["Q3"]) == 0,
        ...
        "q3_exact_ge_5_6": sum(bool(r.get("final_answer_exact")) for r in groups["Q3"]) >= 5,
        "q3_each_provider_ge_2_3": all(q3_by_provider[p]["pass"] >= 2 for p in PROVIDERS),
        "q4_exact_ge_5_6": sum(bool(r.get("final_answer_exact")) for r in groups["Q4"]) >= 5,
    ...
    return {
        ...
        "metrics": {
            ...
            "q3_exact": _rate(groups["Q3"], lambda r: bool(r.get("final_answer_exact"))),
            "q4_exact": _rate(groups["Q4"], lambda r: bool(r.get("final_answer_exact"))),
