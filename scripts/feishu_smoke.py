...
def main() -> int:
    ...
    parser.add_argument(
        "--render-matrix",
        action="store_true",
        help="F5: 发送渲染支持矩阵样本（对照 docs/feishu_render_matrix.md 人工确认）",
    )
    ...
    if args.chat_id:
        ...
        try:
            ...
            print(
                "     内容含：标题/加粗/行内代码/代码块/列表/表格——请在飞书桌面端+手机端确认 markdown 渲染"
            )
        except Exception as exc:  # noqa: BLE001 — 如实记录
...
def _send_render_matrix(rest, chat_id: str) -> None:
    ...
    samples = [
        ...
        (
            "03-行内代码",
            "行内代码 `code_here` 与链接 [项目主页](https://github.com/deyi2026/llm-first-loop)",
        ),
        ("04-有序列表", "有序列表:\n1. 第一项\n2. 第二项\n3. 第三项"),
        ("05-任务列表", "任务列表:\n- [x] 已完成项\n- [ ] 未完成项"),
        ("06-引用块", "> 引用块文本\n\n---\n水平线之上为引用"),
        (
            "07-图片",
            "图片语法测试: ![alt](https://example.com/nonexistent.png)（应显示占位或链接）",
        ),
    ]
