...
TASKS = [
    {
        "id": "home_render",
        "desc": "首页渲染：标题非空 + axtree 有内容",
        "code_tpl": '# 首页渲染检查\nt = goto("{base}/")\nassert t, "标题为空"\ntree = axtree_text()\nassert tree, "无障碍树为空"\nprint("TITLE:", t)\nprint("TREE_CHARS:", len(tree))',
    },
    {
        "id": "file_preview",
        "desc": "文件预览：/api/v1/files/preview 可达",
...
def main() -> None:
    ...
    report = {
        ...
        "total": len(results),
        "passed": ok,
        "pass_rate": round(ok / len(results), 3),
