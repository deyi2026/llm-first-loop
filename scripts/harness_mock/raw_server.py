...
import http.server, subprocess, os, sys, posixpath
...
class Handler(http.server.BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        rp = find_repo(repo)
        if not rp:
            self.send_error(404, f'repo {repo} not found locally'); return
        r = subprocess.run(['git', 'show', f'{commit}:{path}'], cwd=rp,
                          capture_output=True, text=True)
        if r.returncode != 0:
            self.send_error(404, f'git show failed: {r.stderr[:100]}'); return
