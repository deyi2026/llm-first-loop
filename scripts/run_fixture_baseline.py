...
def chat(message: str, session_id=None, timeout=1800):  # 27B 本地 agentic 单轮可跑 10-30 分钟
    ...
    if session_id:
        ...
        body["new_session"] = True  # routes.py:179-180 契约：new_session 优先 engine.session.create()
...
def main():
    ...
    for t in fixture["tasks"]:
        ...
        for i, msg in enumerate(t["messages"], 1):
            ...
            rounds.append({"round": i, "answer_head": ans[:150], "drift": drift,
                           "model_used": data.get("model_used", ""), "secs": round(time.time()-t0, 1)})
            print(f"[{tid}] r{i} drift={drift} secs={round(time.time()-t0,1)} head={ans[:60]!r}", flush=True)
            time.sleep(2)
