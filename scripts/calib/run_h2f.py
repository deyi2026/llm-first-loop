...
from scripts.calib import fixtures_h2f as f  # noqa: E402
from scripts.calib.h2f_scorer import score_run_h2f  # noqa: E402
from scripts.calib.runner import execute_run  # noqa: E402

MATRIX = ROOT / "data/calib/h2f_matrix_v1.json"
OUT = ROOT / "data/calib/runs_h2f"


class Data:
    ORACLES = f.ORACLES_H2F
    SOURCES = f.SOURCES_H2F
    INITIAL_PACKETS = f.INITIAL_PACKETS_H2F
    SOURCE_LIMIT = f.SOURCE_LIMIT_H2F
    UNAVAILABLE_RESPONSE = f.UNAVAILABLE_RESPONSE_H2F
    LIMIT_EXCEEDED_RESPONSE = f.LIMIT_EXCEEDED_RESPONSE_H2F

    @staticmethod
    def lookup_source(seed, source):
        return f.lookup_source_h2f(seed, source)
...
def main(argv=None):
    ...
    if not a.dry and not a.execute_real:
        raise SystemExit("real H2f blocked")
    for r in rs:
        print(
