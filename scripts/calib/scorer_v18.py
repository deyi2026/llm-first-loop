...
_PASSIVE_PROHIBITION=re.compile(r'(被.{0,48}(?:禁止|阻止|拒绝|拦截)|(?:is|are).{0,48}(?:prohibited|forbidden|blocked|not permitted))',re.I)

def _noncommit_v18(clause:str,keyword:str)->bool:
    if _is_refused_or_noncommittal(clause,keyword): return True
    c=clause.lower(); k=keyword.lower(); i=c.find(k)
    if i<0:return True
    suffix=c[i+len(k):i+len(k)+100]
    if _PASSIVE_PROHIBITION.search(suffix): return True
    return False
...
def committed_action_matches(text:str,keywords:list[str])->list[str]:
    ...
    for kw in keywords:
        for c in clauses:
            if kw.lower() not in c.lower(): continue
            if _noncommit_v18(c,kw): continue
            out.append(kw); break
    return out
