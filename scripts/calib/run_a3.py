...
from scripts.calib import fixtures_a3 as f  # noqa: E402
from scripts.calib.runner_a3 import execute_run_a3  # noqa: E402

MATRIX = ROOT / "tests/fixtures/calib/a3_matrix_v1.json"
OUT = ROOT / "data/calib/runs_a3"


class Data:
    ORACLES = f.ORACLES_A3
    SOURCES = f.SOURCES_A3
    INITIAL_PACKETS = f.INITIAL_PACKETS_A3
    SOURCE_LIMIT = f.SOURCE_LIMIT_A3
    UNAVAILABLE_RESPONSE = f.UNAVAILABLE_RESPONSE_A3
    LIMIT_EXCEEDED_RESPONSE = f.LIMIT_EXCEEDED_RESPONSE_A3

    @staticmethod
    def lookup_source(seed, source):
        return f.lookup_source_a3(seed, source)
...
def main(argv=None):
    ...
    if a.run:
        ...
        if idx is None:
            raise SystemExit(f"from-run not found: {a.from_run}")
        rs = rs[idx:]
        rs = rs[: a.count] if a.count is not None else rs
    elif not a.all:
        raise SystemExit("use --all, --run, or --from-run")
    if not a.dry and not a.execute_real:
        raise SystemExit("real A3 blocked until frozen preregistration")
    for r in rs:
        print(
        ...
        print(
            ...
            o["status"],
            "attempt",
            o["tool_attempt_count"],
            "exec",
            o["tool_execution_count"],
            "dup",
            o["duplicate_suppressed_count"],
            "blocked",
            o["budget_blocked_count"],
            "rounds",
            o["rounds_to_final"],
            "lat",
            o["stats"]["latency_s"],
