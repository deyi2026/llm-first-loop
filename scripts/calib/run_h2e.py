...
from scripts.calib import fixtures_h2e as f  # noqa: E402
from scripts.calib.h2e_scorer import score_run_h2e  # noqa: E402
from scripts.calib.runner import execute_run  # noqa: E402

MATRIX = ROOT / "data/calib/h2e_matrix_v1.json"
OUT = ROOT / "data/calib/runs_h2e"


class Data:
    ORACLES = f.ORACLES_H2E
    SOURCES = f.SOURCES_H2E
    INITIAL_PACKETS = f.INITIAL_PACKETS_H2E
    SOURCE_LIMIT = f.SOURCE_LIMIT_H2E
    UNAVAILABLE_RESPONSE = f.UNAVAILABLE_RESPONSE_H2E
    LIMIT_EXCEEDED_RESPONSE = f.LIMIT_EXCEEDED_RESPONSE_H2E

    @staticmethod
    def lookup_source(seed, source):
        return f.lookup_source_h2e(seed, source)
...
def main(argv=None):
    ...
    if a.regrade:
        ...
        for r in rs:
            ...
            o["score"] = score_run_h2e(o)
            p.write_text(json.dumps(o, ensure_ascii=False, indent=2))
            ss.append(o["score"])
        (OUT / f"report_{a.provider or 'all'}.json").write_text(
            json.dumps(
    ...
    if not a.dry and not a.execute_real:
        raise SystemExit("real H2e blocked")
    for r in rs:
        print(
