...
from scripts.calib.s_scorer import score_run_s  # noqa: E402
from scripts.calib.screening_runner import execute_screening_run, snapshot_provider  # noqa: E402
...
def _select(args):
    ...
    if args.run:
        ...
        if not rows:
            raise SystemExit(f"unknown/nonselected run: {args.run}")
    elif args.from_run:
        ids = [r["run_id"] for r in rows]
        if args.from_run not in ids:
            raise SystemExit(f"--from run not in selected block: {args.from_run}")
...
def main(argv=None):
    ...
    if args.regrade:
        scores = []
        for r in _matrix():
            if args.provider and r["provider"] != args.provider:
                continue
        ...
        report = {
            ...
            "mode": "regrade",
            "count": len(scores),
            "matrix_version": "S1-v1",
            "runs": scores,
        }
        suffix = f"_{args.provider}" if args.provider else ""
        (args.out / f"report{suffix}.json").write_text(
    ...
    if not args.dry and not args.execute_real:
        raise SystemExit(
            "refusing provider requests: use --execute-real after approval + smoke gate"
        )
    if args.execute_real:
        ...
        if bad:
            raise SystemExit(
                "real screening blocked: missing/passing smoke artifact for " + ",".join(bad)
            )

    scores = []
    for r in rows:
        print(
            f"[{r['run_id']}] {r['provider']} {r['seed']} {r['variant']} {'dry' if args.dry else 'REAL'}",
            flush=True,
        )
        obj = execute_screening_run(
            r["run_id"],
            r["provider"],
            r["seed"],
            r["variant"],
        ...
        obj["score"] = score_run_s(obj)
        (args.out / f"{r['run_id']}.json").write_text(
            json.dumps(obj, ensure_ascii=False, indent=2), encoding="utf-8"
        )
        scores.append(obj["score"])
        s = obj["score"]
        print(
            f" -> status={obj['status']} task={s['task_success']} novel={s['novel_stage']} fatal={s['fatal_behavior']} unnec={s['unnecessary_verification_count']}"
        )
    report = {
        "generated": datetime.now(UTC).isoformat(),
        "dry": args.dry,
