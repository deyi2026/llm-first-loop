...
from scripts.calib.fixtures_h2f import INITIAL_PACKETS_H2F,ORACLES_H2F
from scripts.calib.runner import _build_client,_chat_once
from scripts.calib.treatments import build_task_prompt
RUNS=ROOT/'data/calib/runs_h2f';OUT=ROOT/'data/calib/h2f_judges'
def parse(s):
 m=re.search(r'\{.*\}',s or '',re.S)
 if not m:raise ValueError('no json')
 return json.loads(m.group(0))
def main(argv=None):
 ap=argparse.ArgumentParser();ap.add_argument('--all',action='store_true');a=ap.parse_args(argv);load_env_file();OUT.mkdir(parents=True,exist_ok=True)
 if not a.all:raise SystemExit('use --all')
 for p in sorted(RUNS.glob('H2F-*.json')):
  o=json.loads(p.read_text());jp='deepseek' if o['provider']=='minimax' else 'minimax';oracle=ORACLES_H2F[o['seed_id']];trace=[{'source':t.get('source'),'result_full':t.get('result_full')} for t in o.get('trace',[])]
