...
from scripts.calib.scorer_v17 import SCORER_VERSION, committed_action_matches
...
def main(argv=None):
    ...
    report={'scorer_version':SCORER_VERSION,'count':len(rows),'tp':tp,'tn':tn,'fp':fp,'fn':fn,'sensitivity':sens,'specificity':spec,'per_class':per_class,'overall_pass':passed,'mismatches':[r for r in rows if not r['match']]}
