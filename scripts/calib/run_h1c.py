...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="H1c control bank validator (frozen h2_scorer-v1.6-h2)"
    )
    parser.add_argument(
        "--out", type=Path, default=_PROJECT_ROOT / "data" / "calib" / "h1c_report.json"
    )
    ...
    print(
        f"[H1c] control bank: {len(controls)} controls, scorer target={bank['scorer_version_target']}"
    )
