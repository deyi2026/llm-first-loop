"""H1c Frozen Scorer Control Bank validator.

用冻结的 h2_scorer-v1.6-h2 对 `data/calib/h1c_control_bank.json` 中 40 条预标注 control trace 打分，
对比 gold，输出 per-class 准确率、字段级 confusion metrics、novel_stage confusion matrix 与
预注册 Gate 判定（REVIEW §7 阈值）。额外输出 `v16_fix_points`（Holdout Discipline 修复点）逐项验证。

用法:
    .venv/bin/python scripts/calib/run_h1c.py
    .venv/bin/python scripts/calib/run_h1c.py --out data/calib/h1c_report.json
...
CONTROL_BANK_PATH = _PROJECT_ROOT / "data" / "calib" / "h1c_control_bank.json"
...
_FIX_ID_RE = re.compile(r"(V\d+-\d+|S\d+-\d+|[A-Z]{2}-\d+)")
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="H1c control bank validator (frozen h2_scorer-v1.6-h2)")
    parser.add_argument("--out", type=Path, default=_PROJECT_ROOT / "data" / "calib" / "h1c_report.json")
    ...
    print(f"[H1c] control bank: {len(controls)} controls, scorer target={bank['scorer_version_target']}")

    detail = []
    for c in controls:
        pred = score_run_h2(_to_run(c))
        gold = c["gold"]
        row = {
    ...
    print(f"[H1c] exact full-field matches: {len(detail) - n_mismatch}/{len(detail)}")

    # v16_fix_points 逐项验证
    print("\n=== v16_fix_points（Holdout Discipline 修复点） ===")
    fix_check: list[dict] = []
    for fp in bank.get("v16_fix_points", []):
        ids = _FIX_ID_RE.findall(fp)
        rows = [r for r in detail if any(r["control_id"] == f"H1C-{i}" for i in ids)]
    ...
    print(f"\n[H1c] OVERALL: {'PASS' if overall else 'FAIL'}")
    if not overall:
        for r in detail:
