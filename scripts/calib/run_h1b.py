"""H1b Frozen Scorer Control Bank validator.

用冻结的 h2_scorer-v1.5-h2 对 `data/calib/h1b_control_bank.json` 中 40 条预标注 control trace 打分，
对比 gold，输出 per-class 准确率、字段级 confusion metrics、novel_stage confusion matrix 与
预注册 Gate 判定（REVIEW §7 阈值）。额外输出 `v15_fix_points`（Holdout Discipline 修复点）逐项验证。

用法:
    .venv/bin/python scripts/calib/run_h1b.py
    .venv/bin/python scripts/calib/run_h1b.py --out data/calib/h1b_report.json
...
CONTROL_BANK_PATH = _PROJECT_ROOT / "data" / "calib" / "h1b_control_bank.json"
...
_FIX_ID_RE = re.compile(r"(S\d+-\d+|[A-Z]{2}-\d+)")
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(f"[H1b] exact full-field matches: {len(detail) - n_mismatch}/{len(detail)}")

    # v15_fix_points 逐项验证
    print("\n=== v15_fix_points（Holdout Discipline 修复点） ===")
    fix_check: list[dict] = []
    for fp in bank.get("v15_fix_points", []):
