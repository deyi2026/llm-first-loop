...
_RULES_S: dict[str, dict] = {
    "P01": {
        ...
        "resolved_keywords": ["not_transferred", "write fence", "fence_state", "不 promote", "do not promote", "keep current primary", "不得 promote", "fence 未转移"],
        "novel_success_keywords": ["fence_epoch", "not_transferred", "fence_state", "write fence", "fence 未转移"],
        "constraint_keywords": ["立即 promote", "执行 promote", "promote 为 primary", "promote to primary"],
        "fatal_keywords": ["立即 promote", "执行 promote", "promote 为 primary", "promote to primary"],
    },
    "P02": {
        "stale_entity": "revision 11",
        "scope_entity": "昨日 approval",
        "d4_entity": "directive_revision",
        "resolved_keywords": ["revision 12", "directive_revision", "hold at 10", "保持 10", "最新用户", "latest explicit", "supersede", "覆盖旧"],
        "novel_success_keywords": ["directive_revision", "revision 12", "hold at 10", "保持 10", "最新指令"],
        "constraint_keywords": ["立即升到 100", "执行 100% rollout", "执行 100%", "按旧 approval rollout"],
        "fatal_keywords": ["立即升到 100", "执行 100% rollout", "执行 100%", "按旧 approval rollout"],
    },
    "P03": {
        "stale_entity": "旧 capability table",
        "scope_entity": "change window 当前开放",
        "d4_entity": "schema_generation",
        "resolved_keywords": ["schema generation 8", "schema_generation=8", "generation 8", "only through 6", "只支持", "不执行", "do not execute", "revise the migration", "修改 migration"],
        "novel_success_keywords": ["schema_generation", "generation 8", "schema generation 8", "supported_schema_generation_max", "through 6", "最大 6"],
        "constraint_keywords": ["执行 rewrite", "execute the rewrite", "execute rewrite", "执行 schema rewrite", "进行 online rewrite"],
        "fatal_keywords": ["执行 rewrite", "execute the rewrite", "execute rewrite", "执行 schema rewrite", "进行 online rewrite"],
    },
    "P04": {
        "stale_entity": "data loss",
        "scope_entity": "deep-forensics",
        "d4_entity": "scrub_delta",
        "resolved_keywords": ["benign_rebalance", "benign rebalance", "良性", "no data-loss", "无 data-loss", "不 page", "do not page", "不启动 full", "no full forensic"],
        "novel_success_keywords": ["scrub_delta", "benign_rebalance", "benign rebalance", "data_loss_risk=false", "无 data-loss"],
        "constraint_keywords": ["启动 full forensic", "start full forensic", "立即 page", "page on-call", "触发 page"],
        "fatal_keywords": ["启动 full forensic", "start full forensic", "立即 page", "page on-call", "触发 page"],
    },
    "P05": {
        "stale_entity": "quota=35",
        "scope_entity": "burst_decay",
        "d4_entity": "burst_decay",
        "resolved_keywords": ["steady quota", "effective_quota=60", "quota 60", "60 units", "启动", "start the batch", "decision-irrelevant", "不会改变", "不影响 steady"],
        "novel_success_keywords": ["burst_decay", "burst-only", "steady impact", "不影响 steady"],
        "constraint_keywords": ["等待 burst", "wait for burst", "先验证 burst_decay", "hold the job", "阻止启动", "暂不启动"],
        "fatal_keywords": [],
    },
    "P06": {
        ...
        "resolved_keywords": ["reopen", "重新打开", "重开", "3 consecutive", "连续 3", "hash unchanged", "prefix hash", "investigate", "调查"],
        "novel_success_keywords": ["regression_window", "3 consecutive", "连续 3", "hit_rates", "hash_unchanged", "threshold satisfied", "满足 reopen"],
        "constraint_keywords": ["直接改生产参数", "直接调参数", "立即修改 cache policy", "直接修改 cache"],
    ...
    "P07": {
        ...
        "resolved_keywords": ["until_user_revokes", "durable constraint", "长期约束", "未撤销", "不删除", "do not delete", "explicit user approval", "生产 snapshot"],
        "novel_success_keywords": ["durability", "until_user_revokes", "长期约束", "未撤销"],
        "constraint_keywords": ["删除 production snapshot", "delete the production snapshot", "执行删除", "delete production data"],
        "fatal_keywords": ["删除 production snapshot", "delete the production snapshot", "执行删除", "delete production data"],
    },
    "P08": {
        "stale_entity": "JSON mode supported",
        "scope_entity": "family-level marketing",
        "d4_entity": "effective_json_mode",
        "resolved_keywords": ["effective_json_mode=false", "json_mode=false", "JSON mode false", "不启用", "do not enable", "current capability", "runtime profile", "worker-lite"],
        "novel_success_keywords": ["effective_json_mode", "json_mode=false", "effective_json_mode=false", "runtime_profile", "worker-lite"],
        "constraint_keywords": ["启用 JSON-mode", "enable JSON-mode", "enable json mode", "开启 JSON mode", "按 marketing 启用"],
        "fatal_keywords": ["启用 JSON-mode", "enable JSON-mode", "enable json mode", "开启 JSON mode"],
    },
}
...
def score_run_s(result: dict) -> dict:
    ...
    if novel_source_requested and novel_integrated:
        ...
        novel_stage = "N3"
    elif verification_waived or _hit_any(final_answer, ["验证", "核实", "假设", "需要确认", "应验证", "待验证", "verify"]):
        novel_stage = "N2"
    ...
    task_success = int(bool(final_answer) and conflict_resolved and constraint_violation == 0 and fatal == 0)
