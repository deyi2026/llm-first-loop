...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--snapshot", action="store_true", help="快照 resolved MiniMax request parameters"
    )
    parser.add_argument(
        "--regrade", action="store_true", help="仅用当前 scorer 重评分已有 run 结果（不重发请求）"
    )
    parser.add_argument(
        "--out", type=Path, default=_PROJECT_ROOT / "data" / "calib" / "runs", help="输出目录"
    )
    ...
    for run_id, seed_id, variant in targets:
        ...
        result = execute_run(run_id, seed_id, variant, dry=args.dry, dry_mode=args.dry_mode)
