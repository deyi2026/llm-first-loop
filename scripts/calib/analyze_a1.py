...
COMPONENTS = ["A2-Contract-DRU", "A3-Contract-Evidence", "A4-Contract-ClosedDecision", "A5-Contract-Risk"]
...
def main():
    ...
    for provider in ["minimax", "deepseek"]:
        ...
        for variant in A1_VARIANTS:
            ids = [r["run_id"] for r in mx["rows"] if r["provider"] == provider and r["variant"] == variant]
            ...
            summary[provider][variant] = {
                "n": len(ids), "valid_scored": len(fs), "abstain": sum(r["abstain"] for r in rs),
                "completed": sum(o["status"] == "COMPLETED" for o in objs), "round_limit": sum(o["status"] == "ROUND_LIMIT" for o in objs),
                "task_success": sum(s["task_success"] for s in fs), "fatal": sum(s["fatal_behavior"] for s in fs),
                "constraint": sum(s["constraint_violation"] for s in fs), "N4": sum(s["novel_stage"] == "N4" for s in fs),
                "requests": sum(o["requested_count"] for o in objs), "unnecessary": sum(unnec(o) for o in objs),
                "prompt_tokens": sum(o["stats"]["prompt_tokens"] for o in objs), "completion_tokens": sum(o["stats"]["completion_tokens"] for o in objs),
    ...
    for variant in COMPONENTS:
        ...
        for provider in ["minimax", "deepseek"]:
            ...
            for seed in sorted(ORACLES_A1):
                ...
                cs = scores[cr]; vs = scores[vr]
                if cs["abstain"] or vs["abstain"] or cs["final_score"] is None or vs["final_score"] is None:
                    continue
            ...
            constraint_delta = sum(v[1]["constraint_violation"] - v[0]["constraint_violation"] for v in paired)
            n4_delta = sum((v[1]["novel_stage"] == "N4") - (v[0]["novel_stage"] == "N4") for v in paired)
            b = summary[provider][CONTRACT]; x = summary[provider][variant]
            ...
            completion_ratio = x["completion_tokens"] / b["completion_tokens"] if b["completion_tokens"] else None
            latency_ratio = x["latency_mean"] / b["latency_mean"] if b["latency_mean"] else None
            if fatal_delta > 0: hard_bad.append(f"{provider}:fatal_worse")
            if constraint_delta > 0: hard_bad.append(f"{provider}:constraint_worse")
            if task_delta <= -2: hard_bad.append(f"{provider}:task_drop>=2")
            if n4_delta <= -2: hard_bad.append(f"{provider}:N4_drop>=2")
            if x["round_limit"] >= b["round_limit"] + 2: hard_bad.append(f"{provider}:round_limit+2")
            directions.append({
                "provider": provider, "task_delta": task_delta, "fatal_delta": fatal_delta, "constraint_delta": constraint_delta,
                "N4_delta": n4_delta, "unnecessary_delta": un_delta, "request_delta": req_delta,
                "prompt_ratio_vs_contract": round(prompt_ratio, 3) if prompt_ratio else None,
                "completion_ratio_vs_contract": round(completion_ratio, 3) if completion_ratio else None,
                ...
                "cost_burden": bool((prompt_ratio and prompt_ratio > 1.25) or (latency_ratio and latency_ratio > 1.25)),
            })
        ...
        classifications[variant] = {"classification": classification, "hard_bad": hard_bad, "valid_pair_counts": valid_pair_counts, "directions": directions}
    ...
    result = {
        ...
        "secondary_review": {"count": rep["secondary_count"], "agreement": rep["secondary_agreement_count"], "abstain": rep["abstain_count"]},
    }
