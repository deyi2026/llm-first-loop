...
from scripts.calib.scorer_v19 import SCORER_VERSION, novel_stage  # noqa: E402

BANK = ROOT / "data/calib/h1f_control_bank.json"
REPORT = ROOT / "data/calib/h1f_report.json"
...
def main():
    ...
    for c in b["controls"]:
        p = novel_stage(
            ...
            c["text"],
            c["truth_phrases"],
            waiver=c.get("waiver", False),
            verification_plan=c.get("verification_plan", False),
        )
        rows.append({**c, "pred": p, "match": p == c["gold"]})
