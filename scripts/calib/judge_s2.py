...
from scripts.calib.fixtures_s2 import INITIAL_PACKETS_S2, ORACLES_S2  # noqa: E402
...
def do_judge(o, provider, case):
    ...
    if p.exists():
        return json.loads(p.read_text())["judge"]
    j = judge_case(case, provider, f"S2-JUDGE-{provider}-{o['run_id']}")
    p.write_text(
        json.dumps(
