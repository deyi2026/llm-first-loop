...
from scripts.calib.fixtures_h2d import INITIAL_PACKETS_H2D,ORACLES_H2D
from scripts.calib.runner import _build_client,_chat_once
from scripts.calib.treatments import build_task_prompt
RUNS=ROOT/'data'/'calib'/'runs_h2d'; OUT=ROOT/'data'/'calib'/'h2d_judges'

def parse(s):
 m=re.search(r'\{.*\}',s or '',re.S)
 if not m: raise ValueError('judge returned no JSON object')
 return json.loads(m.group(0))
def main(argv=None):
 ap=argparse.ArgumentParser(); ap.add_argument('--run'); ap.add_argument('--all',action='store_true'); a=ap.parse_args(argv); load_env_file(); OUT.mkdir(parents=True,exist_ok=True)
 paths=sorted(RUNS.glob('H2D-*.json'))
 if a.run: paths=[RUNS/f'{a.run}.json']
 elif not a.all: raise SystemExit('use --all or --run')
 for p in paths:
  o=json.loads(p.read_text()); provider=o['provider']; judge_provider='deepseek' if provider=='minimax' else 'minimax'; oracle=ORACLES_H2D[o['seed_id']]
  trace=[{'name':t.get('name'),'source':t.get('source'),'result_full':t.get('result_full')} for t in o.get('trace',[])]
  ...
  try: resp=_chat_once(llm,[{'role':'user','content':prompt}],[],f"JUDGE-{o['run_id']}",judge_provider,llm.model)
  finally: llm.close()
  j=parse(resp.content); out={'run_id':o['run_id'],'seed_id':o['seed_id'],'judge_provider':judge_provider,'judge':j}
  (OUT/f"{o['run_id']}.json").write_text(json.dumps(out,ensure_ascii=False,indent=2)); print(o['run_id'],judge_provider,j)
 return 0
