...
from scripts.calib.fixtures_h2h import INITIAL_PACKETS_H2H, ORACLES_H2H  # noqa: E402
...
RUNS = ROOT / "data/calib/runs_h2h"
JDIR = ROOT / "data/calib/h2h_judges"
REPORT = ROOT / "data/calib/h2h_report.json"


def case_for(o):
    oracle = ORACLES_H2H[o["seed_id"]]
    trace = [
        {"round": t.get("round"), "source": t.get("source"), "result_full": t.get("result_full")}
        for t in o.get("trace", [])
    ]
    return {
        "task": build_task_prompt(o["seed_id"], INITIAL_PACKETS_H2H),
...
def main(argv=None):
    ...
    for p in sorted(RUNS.glob("H2H-*.json")):
        ...
        for provider in ["minimax", "deepseek"]:
            ...
            if jp.exists():
                j = json.loads(jp.read_text(encoding="utf-8"))["judge"]
            else:
            ...
            js[provider] = j
            derived[provider] = derive_core_score(case, j, o["status"])
        semantic_agree = all(
            js["minimax"][f] == js["deepseek"][f]
            for f in [
        ...
        derived_agree = all(
            derived["minimax"][f] == derived["deepseek"][f]
            for f in ["task_success", "fatal_behavior", "constraint_violation", "novel_stage"]
        )
        ...
        row = {
            ...
            "judges": js,
            "derived": derived,
            "semantic_agree": semantic_agree,
            "derived_agree": derived_agree,
            "abstain": not (semantic_agree and derived_agree),
            "final_score": None if not (semantic_agree and derived_agree) else derived[primary],
        }
        rows.append(row)
        print(
            o["run_id"],
            "agree",
            semantic_agree and derived_agree,
            "primary",
            primary,
            derived[primary],
            flush=True,
    ...
    n = len(rows)
    agree = sum(not r["abstain"] for r in rows)
    report = {
        "count": n,
        "agreement_count": agree,
        "agreement_rate": agree / n if n else 0,
        "abstain_count": n - agree,
        "rows": rows,
    }
    REPORT.write_text(json.dumps(report, ensure_ascii=False, indent=2), encoding="utf-8")
    print(
        json.dumps(
            {k: report[k] for k in ["count", "agreement_count", "agreement_rate", "abstain_count"]},
            ensure_ascii=False,
        )
    )
    return 0
