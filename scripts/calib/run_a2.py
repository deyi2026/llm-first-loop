...
class Data:
    ORACLES = f.ORACLES_A2
    SOURCES = f.SOURCES_A2
    INITIAL_PACKETS = f.INITIAL_PACKETS_A2
    SOURCE_LIMIT = f.SOURCE_LIMIT_A2
    UNAVAILABLE_RESPONSE = f.UNAVAILABLE_RESPONSE_A2
    LIMIT_EXCEEDED_RESPONSE = f.LIMIT_EXCEEDED_RESPONSE_A2

    @staticmethod
    def lookup_source(seed, source):
        return f.lookup_source_a2(seed, source)
...
def main(argv=None):
    ...
    if not a.dry and not a.execute_real:
        raise SystemExit("real A2 blocked until frozen preregistration")
    for r in rs:
        print(
