"""A2 Full-Slim confirmation generation runner."""
...
from scripts.calib import fixtures_a2 as f
from scripts.calib.runner_a2 import execute_run_a2
MATRIX=ROOT/'data/calib/a2_matrix_v1.json'; OUT=ROOT/'data/calib/runs_a2'
class Data:
 ORACLES=f.ORACLES_A2; SOURCES=f.SOURCES_A2; INITIAL_PACKETS=f.INITIAL_PACKETS_A2
 SOURCE_LIMIT=f.SOURCE_LIMIT_A2; UNAVAILABLE_RESPONSE=f.UNAVAILABLE_RESPONSE_A2; LIMIT_EXCEEDED_RESPONSE=f.LIMIT_EXCEEDED_RESPONSE_A2
 @staticmethod
 def lookup_source(seed,source):return f.lookup_source_a2(seed,source)
DATA=Data()
...
def main(argv=None):
 ...
 if not a.dry and not a.execute_real:raise SystemExit('real A2 blocked until frozen preregistration')
 for r in rs:
  ...
  o=execute_run_a2(r['run_id'],r['seed'],r['variant'],dry=a.dry,provider=r['provider'],data=DATA); o['provider']=r['provider'];o['matrix_block_seq']=r['block_seq'];p.write_text(json.dumps(o,ensure_ascii=False,indent=2),encoding='utf-8')
