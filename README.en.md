...
> **License**: [Apache-2.0](LICENSE) ｜ **Version**: 0.6.10 ｜ **Status**: Open-source framework-ization (Track B) in progress ｜ **中文版**: [README.md](README.md)
...
- **Program minimalism + necessary support**: judgments the AI can make autonomously with documentation rules (`docs/ai_rules.md`) stay out of code wherever possible. Code focuses on capabilities the model cannot reliably provide by itself: real execution/persistence, protocol and authorization integrity, catastrophic-safety and physical-resource bounds, crash recovery, and non-repeatable side-effect protection.
...
- **Evolution proposals land and execute automatically** (M12 deepening + M16/M17 audit): the AI can submit architecture evolution proposals via `submit_evolution` (a pure suggestion channel; the receipt includes "awaiting evolve-review review" guidance); `EVOLVE_LOCAL_EXEC` three-tier permission (0=proposals only / 1=whitelisted partial execution / 2=full execution); after a human `evolve-review <id> accepted`, execution proceeds automatically per the permission tier (state advancement + audit + execution guidance); **the execution actions/verification/rollback are done autonomously by the AI** (carried by the correction tools `adjust_strategy`/`retry_tool`/`refresh_config` under the sub-rules of RULE-AI-06; the program does not execute/verify/roll back for the AI, and verify_result=unverified is labeled truthfully); after execution the AI registers "done + verification conclusion" via the `evolution_complete` tool (executing→executed); boundary-crossing evolutions are registered manually via the CLI `evolve-complete` (executor=human); executing/pending evolution state remains queryable and auditable instead of being automatically injected into ordinary user prompts; the full-chain audit log `evolution_exec_log` is searchable
...
- **CI + versioning** (T7/B11): GitHub Actions three-gate checks (pytest/ruff/pyright) + nightly real evaluation; semantic versioning v0.6.10; Release Drafter (PR titles auto-categorized into the changelog) + tag-triggered gate re-check + Release draft (institutionalized release cadence)
