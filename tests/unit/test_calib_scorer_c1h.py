...
_H1C_BANK_PATH = (
    Path(__file__).resolve().parent.parent.parent / "data" / "calib" / "h1c_control_bank.json"
)

if not _H1C_BANK_PATH.exists():
    # CI checkout 无 data/ 运行时数据（.gitignore 设计），h2_scorer 模块级读取会炸 collection
    pytest.skip(
        "data/calib/h1c_control_bank.json 不存在（CI 无运行时 calib 数据）", allow_module_level=True
    )


def _result(
    seed: str, answer: str, sources: list[str] | None = None, reasoning: str | None = None
) -> dict:
    trace = [
        {"round": 1, "name": "request_fixture", "source": source} for source in (sources or [])
    ]
    return {
        "run_id": "TEST-H2B",
        "seed_id": seed,
...
def test_scorer_version_v16_h2() -> None:
    s = score_run_h2(
        _result("H09", "以 edge_config 为准：edge_ttl=120。", ["fixture://H09/edge_config"])
    )
