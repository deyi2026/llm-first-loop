...
def test_s2_seeds_and_sources_are_complete():
 assert set(INITIAL_PACKETS_S2)==set(ORACLES_S2)==set(SOURCES_S2)=={f'E{i:02d}' for i in range(1,9)}
...
def test_s2_matrix_is_balanced_and_secondary_sample_stratified():
 m=json.loads((ROOT/'data/calib/s2_matrix_v1.json').read_text());rows=m['rows'];assert len(rows)==48
 for p in ['minimax','deepseek']:
  for v in ['V0-Baseline','V1-Contract','V2-Full']:
   rr=[r for r in rows if r['provider']==p and r['variant']==v];assert len(rr)==8
   selected=[x for x in m['secondary_review_runs'] if any(r['run_id']==x for r in rr)];assert len(selected)==2
