...
def test_shape():
    assert len(INITIAL_PACKETS_A2) == 8 and len(A2_VARIANTS) == 4 and SOURCE_LIMIT_A2 == 2


def test_new_seed_ids():
    assert set(INITIAL_PACKETS_A2) == {f"G{i:02d}" for i in range(1, 9)}


def test_sources():
    for s, o in ORACLES_A2.items():
        avail = set(INITIAL_PACKETS_A2[s]["sources"])
...
def test_no_truth_hint():
    for p in INITIAL_PACKETS_A2.values():
...
def test_slim_smaller_than_full():
    assert len(build_system_prompt_a2("B2-Full-Slim-v1")) < 0.8 * len(
        build_system_prompt_a2("B3-Full-Reference")
    )


def test_variant_blind_task():
    for s in INITIAL_PACKETS_A2:
        assert len({build_task_prompt(s, INITIAL_PACKETS_A2) for _ in A2_VARIANTS}) == 1


def test_matrix():
    m = json.loads(MATRIX.read_text())
    assert len(m["rows"]) == 64 and m["randomization_seed"] == 202608261030
...
def test_dry_runner_restores_builder():
    import scripts.calib.fixtures_a2 as f
    ...
    data = type(
        ...
        {
            "ORACLES": ORACLES_A2,
            "INITIAL_PACKETS": INITIAL_PACKETS_A2,
            "SOURCE_LIMIT": 2,
            "UNAVAILABLE_RESPONSE": "SOURCE_NOT_AVAILABLE",
            "LIMIT_EXCEEDED_RESPONSE": "SOURCE_LIMIT_EXCEEDED",
            "lookup_source": staticmethod(f.lookup_source_a2),
        },
    )()
    o = execute_run_a2(
        "A2-DRY-TEST", "G01", "B2-Full-Slim-v1", dry=True, provider="minimax", data=data
    )
