...
from scripts.evidence.r13.long_session_stress import (
    CODES,
    STRESS_BUDGET,
    paired_gates,
    per_mode_gates,
...
def _base(mode: str) -> dict:
    return {
        ...
        "unexplained_cache_boundary_backjumps": 3,
        ...
        "tool_non_success_count": 4,
        "unresolved_run_errors": 0,
    }


def test_r13_sources_and_scenario_are_fresh_and_no_anti_repeat_instruction() -> None:
...
def test_r13_per_mode_gates_keep_cache_boundary_backjump_diagnostic_only() -> None:
    report = _base("enforce")
    assert all(per_mode_gates(report).values())
    report["unexplained_cache_boundary_backjumps"] = 99
    assert all(per_mode_gates(report).values())
    report["unresolved_run_errors"] = 1
    assert per_mode_gates(report)["no_unresolved_run_error"] is False


def test_r13_paired_gates_measure_physical_reads_not_declared_fallbacks() -> None:
    off = _base("off")
