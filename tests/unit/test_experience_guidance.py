...
def test_no_memory_store_default_template():
    """无经验库 → 默认模板（零回归）."""
    reg = ToolRegistry(tool_guidance_mode="on")  # memory_store=None
    r = reg._result(ToolResultStatus.FAILURE, _fail_call(), "[文件不存在] /x", duration_ms=1.0)
    assert r.guidance_extra == ""  # 无经验注入
    msg = tool_result_to_message(r, tool_guidance_mode="on")
...
def test_no_hit_default_template():
    ...
    reg = ToolRegistry(memory_store=_FakeMemory([]), tool_guidance_mode="on")
    r = reg._result(ToolResultStatus.FAILURE, _fail_call(), "[文件不存在] /x", duration_ms=1.0)
...
def test_search_exception_fail_open():
    ...
    reg = ToolRegistry(memory_store=_Boom(), tool_guidance_mode="on")
...
def test_default_off_does_not_query_or_mark_experience(monkeypatch):
    ...
    reg = ToolRegistry(memory_store=_ExplodingMemory(), tool_guidance_mode="off")
    r = reg._result(
...
def test_experience_switch_default_follows_main():
    """experience_guidance_enabled=None（默认）→ 跟随主开关：主关则经验也不注入."""
    from llm_loop.memory.store import MemoryEntry
    ...
    msg = tool_result_to_message(r, failure_guidance_enabled=False, tool_guidance_mode="on")  # 未显式传经验开关
