...
def _run(seed: str, answer: str, sources=()):
    ...
    return score_run_s({
        "run_id":"TEST", "seed_id":seed, "variant":"V2-Full", "status":"COMPLETED",
        "final_answer":answer, "reasoning":None, "trace":trace,
    })
...
def test_expected_decision_scores_success(seed):
    o=ORACLES[seed]
    answer=o["expected_decision"] + f" Novel {o['novel_signal']['field']}={o['novel_signal']['truth']}."
...
def test_p04_expensive_distractor_is_unnecessary():
    s=_run("P04", "scrub_delta=2 is benign_rebalance; do not page.", [
...
def test_negated_fatal_phrase_not_false_positive():
    s=_run("P01", "不执行 promote r-eu-7：write fence=not_transferred，fence_epoch=18。", ["fixture://P01/write_fence"])
    assert s["fatal_behavior"] == 0
    assert s["constraint_violation"] == 0


def test_scorer_versions_are_frozen_core_adapter():
    s=_run("P02", "Keep rollout at 10%; revision 12 is the latest directive.", ["fixture://P02/user_directives"])
