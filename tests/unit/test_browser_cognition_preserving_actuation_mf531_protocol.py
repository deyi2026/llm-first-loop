...
PROTOCOL_PATH = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf531/protocol.py"
_spec = importlib.util.spec_from_file_location("mf531_protocol", PROTOCOL_PATH)
...
def test_mf531_identity_and_plan_are_frozen() -> None:
    assert protocol.SCHEMA == "smc.browser_cognition_preserving_actuation_mf531.v0.1"
    assert protocol.IMPLEMENTATION_COMMIT == "1cb9ad2bddd8b44ca31dd38ff1b30c9dd42c01e0"
    plan = protocol.build_plan()
    assert len(plan) == 6
    assert [row["index"] for row in plan] == [1, 2, 3, 4, 5, 6]
    assert all(row["arm"] == "root_direct_perceive_operate" for row in plan)
...
def test_mf531_gate_accepts_clean_six_row_perceive_operate_treatment() -> None:
...
def test_mf531_gate_rejects_perceive_contract_failure_or_error() -> None:
...
def test_mf531_gate_rejects_operate_contract_failure_or_error() -> None:
    rows = _passing_records()
...
def test_mf531_gate_rejects_schema_repair() -> None:
    rows = _passing_records()
...
def test_mf531_gate_rejects_ground_probe_amplification() -> None:
    rows = _passing_records()
    rows[3]["worker"]["ground_probe_amplification_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf531_gate_rejects_duplicate_successful_mutation() -> None:
    rows = _passing_records()
    rows[4]["worker"]["duplicate_successful_mutation_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf531_gate_rejects_invalid_first_browser_call_or_security_agent() -> None:
    rows = _passing_records()
...
def test_mf531_gate_rejects_hidden_atomic_or_boundary_authority() -> None:
    rows = _passing_records()
...
def test_mf531_gate_rejects_unparsed_or_automatic_retry() -> None:
    rows = _passing_records()
