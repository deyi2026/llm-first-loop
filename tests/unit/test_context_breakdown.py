...
def test_breakdown_tool_schema_chars_in_loop(build_test_engine):
    """R1 修复: loop 中 last_breakdown 的 tool_schema chars > 0（修复前恒为 0）.

    EVO 前会话重构: engine._last_breakdown → _run_state().last_breakdown（桶字段）。
    """
    engine, _fake = build_test_engine([{"content": "done"}])
    engine.run("test-session", "你好")
    bd = getattr(engine._run_state(), "last_breakdown", None)
