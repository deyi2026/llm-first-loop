...
def _event(seq: int, kind: str, second: float, payload: dict) -> dict:
    ...
    return {
        ...
        "session_id": "s1",
        "type": kind,
        "ts": base.replace(microsecond=0).isoformat().replace("+00:00", "Z") if second == 0 else (
            datetime.fromtimestamp(base.timestamp() + second, tz=UTC).isoformat()
        ),
        "payload": payload,
    }
...
def test_gpu_equivalent_time_is_time_weighted() -> None:
    ...
    samples = [
        ...
        {"_dt": datetime.fromtimestamp(base.timestamp() + 1.0, tz=UTC), "device_utilization_pct": 0},
    ]
    got = mod.summarize_gpu_interval(samples, base, datetime.fromtimestamp(base.timestamp() + 1.0, tz=UTC))
...
def test_analyzer_keeps_novelty_mechanical_and_out_of_erdc_score() -> None:
    ...
    events = [
        ...
        _event(3, "tool.execution.started", 2.1, {"round": 1, "execution_id": "x", "tool_name": "probe"}),
        _event(4, "tool.execution.finished", 2.6, {
            "round": 1,
            "execution_id": "x",
            "tool_name": "probe",
            "status": "success",
        ...
        _event(7, "tool.execution.started", 4.1, {"round": 2, "execution_id": "y", "tool_name": "probe"}),
        _event(8, "tool.execution.finished", 4.2, {
            "round": 2,
            "execution_id": "y",
            "tool_name": "probe",
            "status": "success",
    ...
    base = datetime(2026, 9, 8, tzinfo=UTC)
    samples = [
        {"_dt": datetime.fromtimestamp(base.timestamp() + n * 0.5, tz=UTC), "device_utilization_pct": 100}
        for n in range(10)
    ]
