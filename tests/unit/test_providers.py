...
def test_chat_template_reasoning_effort_map_is_model_owned_and_passed() -> None:
    raw = json.dumps(
        {
            "local": {
    ...
    assert spec.reasoning_effort_map == {
        "low": "low",
        "medium": "medium",
        "high": "medium",
        "max": "xhigh",
...
def test_invalid_chat_template_reasoning_effort_map_fails_open() -> None:
    raw = json.dumps(
        {
            "local": {
                "base_url": "http://127.0.0.1:8901/v1",
