...
PROTOCOL_PATH = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf534/protocol.py"
_spec = importlib.util.spec_from_file_location("mf534_protocol", PROTOCOL_PATH)
...
def test_mf534_identity_and_plan_are_frozen() -> None:
    assert protocol.SCHEMA == "smc.browser_cognition_preserving_actuation_mf534.v0.1"
    assert protocol.IMPLEMENTATION_COMMIT == "d644e379a250f9787f51c4d16967828972a4eecd"
    plan = protocol.build_plan()
    assert len(plan) == 6
    assert [row["index"] for row in plan] == [1, 2, 3, 4, 5, 6]
    assert all(row["arm"] == "grounding_ref_object_wait" for row in plan)
...
def test_mf534_gate_accepts_clean_six_row_perceive_operate_treatment() -> None:
...
def test_mf534_gate_rejects_perceive_contract_failure_or_error() -> None:
...
def test_mf534_gate_rejects_operate_contract_failure_or_error() -> None:
    rows = _passing_records()
...
def test_mf534_gate_rejects_schema_repair() -> None:
    rows = _passing_records()
...
def test_mf534_gate_rejects_ground_probe_amplification() -> None:
    rows = _passing_records()
    rows[3]["worker"]["ground_probe_amplification_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf534_gate_rejects_duplicate_successful_mutation() -> None:
    rows = _passing_records()
    rows[4]["worker"]["duplicate_successful_mutation_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf534_gate_rejects_invalid_first_browser_call_or_security_agent() -> None:
    rows = _passing_records()
...
def test_mf534_gate_rejects_hidden_atomic_or_boundary_authority() -> None:
    rows = _passing_records()
...
def test_mf534_gate_rejects_unparsed_or_automatic_retry() -> None:
    rows = _passing_records()
...
def test_mf534_browser_capability_pair_is_frozen() -> None:
...
def _load_old_protocol(module_name: str):
    old_path = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf533/protocol.py"
...
def test_mf534_fixture_is_byte_identical_to_mf533() -> None:
    import hashlib
    old = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf533/fixture_server.py"
    new = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf534/fixture_server.py"
...
def test_mf534_tasks_prompts_rows_and_oracles_are_identical_to_mf533() -> None:
    old = _load_old_protocol("mf533_protocol_for_invariance")
...
def test_mf534_hard_gate_is_identical_to_mf533_except_schema_identity() -> None:
    old = _load_old_protocol("mf533_protocol_for_gate")
