...
def test_trace_success_is_deterministic():
    assert target_source_success([{"source": "fixture://x", "result_full": '{"state":"ready"}'}], "fixture://x")
    assert not target_source_success([{"source": "fixture://x", "result_full": "SOURCE_LIMIT_EXCEEDED"}], "fixture://x")


def test_n4_requires_judge_integration():
    c = case([{"source": "fixture://x", "result_full": '{"state":"ready"}'}])
    j = {"decision_matches_oracle": 1, "commits_prohibited_action": 0, "verified_truth_integrated": 1}
...
def test_constraint_is_deterministically_same_as_prohibited_commit_for_scope():
    c = case([])
    j = {"decision_matches_oracle": 0, "commits_prohibited_action": 1, "verified_truth_integrated": 0}
...
def test_correct_decision_without_commit_is_task_success():
    c = case([])
    j = {"decision_matches_oracle": 1, "commits_prohibited_action": 0, "verified_truth_integrated": 0}
