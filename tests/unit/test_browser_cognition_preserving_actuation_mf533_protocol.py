...
PROTOCOL_PATH = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf533/protocol.py"
_spec = importlib.util.spec_from_file_location("mf533_protocol", PROTOCOL_PATH)
...
def test_mf533_identity_and_plan_are_frozen() -> None:
    assert protocol.SCHEMA == "smc.browser_cognition_preserving_actuation_mf533.v0.1"
    assert protocol.IMPLEMENTATION_COMMIT == "14dfdb407a098d4efa3c9c16476dd0a5239ceb39"
    plan = protocol.build_plan()
    assert len(plan) == 6
    assert [row["index"] for row in plan] == [1, 2, 3, 4, 5, 6]
    assert all(row["arm"] == "b1_direct_dom_text_a_prime_url_role" for row in plan)
...
def test_mf533_gate_accepts_clean_six_row_perceive_operate_treatment() -> None:
...
def test_mf533_gate_rejects_perceive_contract_failure_or_error() -> None:
...
def test_mf533_gate_rejects_operate_contract_failure_or_error() -> None:
    rows = _passing_records()
...
def test_mf533_gate_rejects_schema_repair() -> None:
    rows = _passing_records()
...
def test_mf533_gate_rejects_ground_probe_amplification() -> None:
    rows = _passing_records()
    rows[3]["worker"]["ground_probe_amplification_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf533_gate_rejects_duplicate_successful_mutation() -> None:
    rows = _passing_records()
    rows[4]["worker"]["duplicate_successful_mutation_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf533_gate_rejects_invalid_first_browser_call_or_security_agent() -> None:
    rows = _passing_records()
...
def test_mf533_gate_rejects_hidden_atomic_or_boundary_authority() -> None:
    rows = _passing_records()
...
def test_mf533_gate_rejects_unparsed_or_automatic_retry() -> None:
    rows = _passing_records()
...
def test_mf533_browser_capability_pair_is_frozen() -> None:
...
def _load_old_protocol(module_name: str):
    old_path = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf532/protocol.py"
    old_spec = importlib.util.spec_from_file_location(module_name, old_path)
...
def test_mf533_fixture_is_byte_identical_to_mf532() -> None:
    import hashlib
    old = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf532/fixture_server.py"
    new = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf533/fixture_server.py"
...
def test_mf533_tasks_prompts_rows_and_oracles_are_identical_to_mf532() -> None:
    old = _load_old_protocol("mf532_protocol_for_invariance")
    assert protocol.ROWS == old.ROWS
    assert set(protocol.TASKS) == set(old.TASKS)
    for task_id in protocol.TASKS:
        assert protocol.TASKS[task_id].prompt_template == old.TASKS[task_id].prompt_template
        assert protocol.TASKS[task_id].expected == old.TASKS[task_id].expected
        states = [
            {"events": []},
            {"events": [{"kind": "commit"}]},
            {"events": [{"kind": "save_code", "value": "AB-7319"}]},
            {"events": [{"kind": "ready_click"}]},
            {"events": [{"kind": "early_click"}]},
        ]
        for state in states:
            assert protocol.judge(task_id, state) == old.judge(task_id, state)


def test_mf533_hard_gate_is_identical_to_mf532_except_schema_identity() -> None:
    old = _load_old_protocol("mf532_protocol_for_gate")
    variants = [_passing_records()]
    for key in (
        "perceive_tool_failure_count", "perceive_tool_error_count",
        "operation_tool_failure_count", "operation_tool_error_count",
        "get_tool_schema_count", "protocol_repair_episode_count",
        "ground_probe_amplification_count", "duplicate_successful_mutation_count",
        "direct_atomic_browser_call_count", "task_completion_violation_count",
        "undeclared_boundary_continuation_count", "operation_unparsed_result_count",
        "operation_automatic_retry_true_count",
    ):
        rows = _passing_records()
        rows[0]["worker"][key] = 1
    ...
    for rows in variants:
        new_gate = protocol.qualification_gate(rows)
        old_gate = old.qualification_gate(rows)
        assert new_gate.pop("schema") != old_gate.pop("schema")
