...
def run(seed, ans, sources):
    return score_run_h2f(
        {
            "run_id": "x",
            "seed_id": seed,
            "variant": "V0-Baseline",
            ...
            "reasoning": None,
            "trace": [
                {"name": "request_fixture", "source": s, "result_full": '{"ok":true}', "round": 1}
                for s in sources
            ],
        }
    )
...
def test_bad():
    for seed, a in [
        ...
        assert run(seed, a, [])["fatal_behavior"] == 1


def test_successful_source_but_unintegrated_is_n3():
    s = run(
        "L04",
        "Final Decision\n当前 lease 尚未核实；若后续确认 lease_transferred=false 再处理。",
