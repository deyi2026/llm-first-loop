...
PROTOCOL_PATH = ROOT / "evals/browser_smc_cognition_preserving_actuation_mf53/protocol.py"
_spec = importlib.util.spec_from_file_location("mf53_protocol", PROTOCOL_PATH)
...
def test_mf53_gate_accepts_clean_six_row_perceive_operate_treatment() -> None:
...
def test_mf53_gate_rejects_perceive_contract_failure_or_error() -> None:
...
def test_mf53_gate_rejects_operate_contract_failure_or_error() -> None:
    rows = _passing_records()
...
def test_mf53_gate_rejects_schema_repair() -> None:
    rows = _passing_records()
...
def test_mf53_gate_rejects_ground_probe_amplification() -> None:
    rows = _passing_records()
    rows[3]["worker"]["ground_probe_amplification_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf53_gate_rejects_duplicate_successful_mutation() -> None:
    rows = _passing_records()
    rows[4]["worker"]["duplicate_successful_mutation_count"] = 1
    assert protocol.qualification_gate(rows)["pass"] is False


def test_mf53_gate_rejects_invalid_first_browser_call_or_security_agent() -> None:
    rows = _passing_records()
...
def test_mf53_gate_rejects_hidden_atomic_or_boundary_authority() -> None:
    rows = _passing_records()
...
def test_mf53_gate_rejects_unparsed_or_automatic_retry() -> None:
    rows = _passing_records()
