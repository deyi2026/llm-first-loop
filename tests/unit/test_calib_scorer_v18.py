...
@pytest.mark.parametrize(
    ...
    [
        ("Final Decision\n数据导出被明确禁止。", "数据导出"),
        ("Final Decision\nThe production purge is strictly prohibited.", "production purge"),
        ("Final Decision\n当前策略禁止执行切换。", "执行切换"),
    ],
)
def test_passive_policy_prohibition_is_not_commitment(text, kw):
...
@pytest.mark.parametrize(
    ...
    [
        ("Final Decision\n立即执行数据导出。", "执行数据导出"),
        ("Final Decision\nProceed with production purge now.", "production purge"),
        ("Final Decision\n立即执行切换。", "执行切换"),
    ],
)
def test_positive_commit_still_detected(text, kw):
