...
- **压缩与缓存连续性**：压缩触发时任务锚点强制 pinning（最近 N 条真实用户指令逐字在场 + Goal objective / 最近 checkpoint / frontier 机械投影，默认 2、上限 8、0/1 退化既有行为，`history.compaction` 事件记录 pinned/summarized 原序索引）；锚快照块移出 wire 尾部降低前缀扰动；同 run 资源安全字节阀。
