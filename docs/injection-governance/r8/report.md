...
R8.6 Tool Eligibility + Recovery audit = PASS
R8.7 Dynamic Tool Eligibility + Runtime Health + Typed Recovery = PASS
R8 behavior canary = NOT STARTED（仍被其余 Prompt Eligibility blockers 冻结）
R9 = NOT STARTED
```

R8.4 的 owner 原则是 **resolved is retrievable, not injectable**；R8.6/R8.7 将同一哲学扩展到工具面：**available is discoverable, not necessarily injectable**。R8.7 已把 61-tool 默认云端面收敛为 stable CORE9 + current-required tail，并提供 `get_tool_schema` 按需 discovery、Playwright runtime quarantine、MCP zero-capability connection retirement，以及 `web_fetch` typed recovery 标杆。clean-source pre-commit simple surface=9 tools / 3,425 raw lazy chars（-84.9% vs all 22,699）。这只关闭 E31 `tool_schemas` surface；legacy resolved history、model-switch、Evidence Manifest、legacy/unresolved packet memory、local hint、unknown producer、round-exhaustion 等 blocker 仍存在，所以不得直接做 model-profile behavior canary。权威清单/证据见 `docs/injection-governance/eligibility/` 与 `docs/injection-governance/tool-eligibility/`。
