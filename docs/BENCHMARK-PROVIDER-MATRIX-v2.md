# AI Operating Benchmark — Multi-Provider Matrix v2

> 文档类型：跨模型/跨 Provider 泛化验收规范
> 状态：v2 Draft / Pre-Experiment
> 核心原则：**MiniMax 只是一个 calibration provider，不是 Architecture 的代表模型。**
> Supersedes：`BENCHMARK-PROVIDER-MATRIX-v1.md`（v1 下执行实验请求数 = 0，无污染）
> v2 变更（2026-08-25）：补 P2 Scorer Reasoning-Field Policy（§15.1）、P3 Reasoning-Mode Confounding Control（§15.2）、P4 Stage S Completion Criteria（§5）
...
**Scorer Reasoning-Field Policy（v2 补）**：thinking 模型（DeepSeek-v4-flash）输出含 reasoning 字段，
C1 的 scorer 校准必须先在 §15.1 的 A/B/C 三选一中声明该字段的处理方式，否则 C1 结论中
"过度反思/Decision Latency"类指标（§15）不可观测，须如实标注限制。
...
**S Completion Criteria（v2 补）**：至少满足以下才算"跨 provider screening 完成"：

```text
1. 两个 Anchor provider（MiniMax-M3 + DeepSeek-v4-flash）的 Baseline/Contract/Full 全部跑完；
2. ≥1 个 holdout provider（Kimi 或 GLM）已完成 registry resolution 并跑完 Baseline/Contract/Full；
3. 每个 run 的 thinking_mode 已作为协变量记录在 run metadata 中（§15.2）。
```

未满足前，S 阶段结论只能标注 `anchor-only screening`，不得声称"跨 provider 方向一致性"。
...
# 15.1 Scorer Reasoning-Field Policy（v2 补）

thinking 模型（如 deepseek-v4-flash）输出含 reasoning 字段，non-thinking 模型（如 MiniMax-M3）无此字段。

C1 起，scorer 对 reasoning 字段的处理必须三选一并写入 C1 pre-registration：

```text
A. ignore    —— scorer 只看 final answer；reasoning 质量不进入任何 metric
B. verbosity —— reasoning 长度/反思次数计入 Decision Latency / Unnecessary Verification 指标
C. separate  —— reasoning 作为独立评分维度（quality rubric 单列）
```

默认：**B（verbosity）**——与 §15 的 interaction 观察目标一致。

若选 A（ignore），则"过度反思/Decision Latency"类指标在 C1 上不可观测，
C1 结论必须如实标注该限制（不能静默缺指标）。

C0（MiniMax-M3, non-thinking）无 reasoning 字段，本政策不适用，但 C1 前必须完成声明。

---

# 15.2 Reasoning-Mode Confounding Control（v2 补）

当前两个 anchor provider 与 reasoning mode **完全混杂**：MiniMax-M3 = non-thinking，
DeepSeek-v4-flash = thinking。provider 效应与 reasoning-mode 效应无法在 Anchor 内分离。

处理规则：

```text
1. S 阶段起，每个 run metadata 必须记录 thinking_mode（true/false），作为协变量参与跨 provider 分析；
2. 跨 provider 归因时，优先检验 treatment × thinking_mode interaction，而非直接归因 provider；
3. 若发现 anchor 间效应差异，禁止写成"provider A 有效 / provider B 无效"——
   必须同时报告"该差异是否可由 reasoning-mode 解释"；
4. 解混杂优先路径：
   同 provider 双模式（同模型开/关 thinking）> 更多 thinking/non-thinking 组合 provider；
5. C1 选择 B（verbosity）时，thinking_mode 协变量以 scoring 原始字段（reasoning 长度/次数）为输入，
   不以模型名代理。
```
