...
> 状态：**R6 / P1-A + P1-B + P1-C 已验证完成**（修订记录见附录 A）
> 日期：2026-09-03
...
> 当前修订：**R6 认识论诚实（Epistemic Honesty）**——把“诚实”提升为全模型共同架构原则：当前可核事实先取当前证据，训练先验/历史经验只作背景或假设；证据必须匹配所声称的对象、版本和运行层，无法核验则明确不确定性。该原则 provider-agnostic，不新增程序语义强制器。R5 的 Rule-first / Prompt Authority 裁决继续有效；历史 R2/R8 章节仅作演进证据。
