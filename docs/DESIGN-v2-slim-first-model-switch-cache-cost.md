...
> 状态: **方案 v2（待审；含时序修正）** | 2026-08-20 | 关联: `docs/ARCHITECTURE-cache-stable-rules.md`（缓存稳定总纲）、`docs/DESIGN-v3-ai-driven-context-switch.md`（AI 主导版，v2 的上位方案）
...
>
> **⚠️ 时序修正声明（2026-08-20，拷问实证）**：
> 代码时序 `build(417) → routing(444) → 切换检测(452)` —— 452 在 build **之后**，
> 切换首轮 build 用的仍是**旧 provider 锚点**（全量/旧尾部）。因此：
> - **首轮全量 miss 是物理事实**（build→routing→452 顺序决定，非本方案缺陷）
> - **瘦身实际从第二轮起生效**（452 写的新锚点，第二轮 build 才读到）
> - 本文 §1.1/§1.2 "第一轮只送尾部短历史" 是**设计意图**，实际为"首轮贵一次、第二轮起省 ~80%"
> - 上游方案 v3（AI 主导）同样受此时序约束，已在其 §0 如实声明
