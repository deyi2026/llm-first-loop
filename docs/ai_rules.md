...
⑤ **结论固化**：关键结论/已确立事实即时写入 [[memory]] 或归档（规则五 / 规则十⑥）——长会话稀释注意力，状态与记忆是唯一可靠信息源。
...
**工具输出截断（2026-08-18 对齐 DSH——缓存命中）**：execute_command / read_file / web_search
超 3000 字符输出截断为首尾保留（`[输出已截断]` 标注 + 完整原文落盘 `data/audit/tool_outputs/`，
可 `read_file` 落盘路径或 `search_archive` 检索全文——零信息丢失；`TOOL_TRIM_MAX` 可调）。
使用须知：
- 需要完整输出时：`read_file` 落盘路径 / 对原文件用 offset/limit 分段——**不要重跑/重读全量**（尾部新增体积 = 缓存命中关键，重复全量拉低命中）。
- 落盘文件在审计目录（data/ 不入库）——检索关键词按命令/文件名（截断标注内有提示词）。
...
- **(a) 工具输出截断**（`execute_command`/`read_file`/`web_search` 超 3000 字符，`tools/trim.py:86`）：完整原文落盘 `data/audit/tool_outputs/`（显式文件），`read_file` 落盘路径取回——**不走 search_archive**。
...
当会话重启、上下文不完整，或记忆与当前对话事实不一致时，先恢复**可验证状态**再继续执行，禁止凭印象重建任务：

1. **事件日志是真相源**：`data/event_logs/<session_id>.jsonl` 按消息即时追加，是中断恢复时的完整消息事实源；仅在检测到缺口时读取缺失段。
2. **核对一次即继续**：用 `read_file` 核对缺失段一次，找回目标/已完成/下一步后立即继续；已确认内容不得换查询词、目录或工具反复检索。
3. **工作区归属必须显式**：涉及路径时先确认主区/镜像归属；镜像为主区同级 `*-mirror`，相对路径以当前 `workspace_base()` 为准，禁止凭记忆猜绝对路径。
4. **已知缺失路径停止探测**：若 `read_file` 失败回执带 `[路径登记]` 已知不存在提示，停止对同一路径换深度/换目录/换工具重复求证；改用 `search_files` 定位或向用户求证。
5. **恢复不越过硬边界**：事件日志只用于恢复事实，不替代 C1-C6 协议、安全审批或镜像变更门禁；恢复后仍按原边界继续。

**程序角色**：事件日志即时落盘、检测内存/日志计数差异并注入一次 `[会话中断恢复]` 事实提示；是否读取、如何继续由 AI 基于规则自主执行。
...
4. **删配置前验证**：先 grep 确认读取路径与门控（none/off 下是否仍读），只删确认无行为影响的；开关保留（显式声明优于隐式默认）。
