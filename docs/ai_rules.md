...
**变更门禁（2026-08-20 用户批准，强制）**：涉及 LFL 自身代码/文件（src/、docs/ai_rules.md、prompt.py、.env、scripts/、tests/、webui/）的修改，一律先在镜像工作区（镜像: 同项目目录的 llm-first-loop-mirror，端口 8903）修改并验证，产出 diff 与验证证据后提交用户审批，批准后才能应用到主区并重启；详见 `docs/MIRROR-workspace-protocol.md`。例外：data/ 运行时状态（记忆/经验/日志/会话）属正常运行不走镜像；用户明确指示立即执行时豁免但事后补验证。
