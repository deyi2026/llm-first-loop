...
### E2. dca86eef Web 端模型全限定名透传 LLM 400 ✅ 已修复（M50 覆盖, 2026-08-20 核实）
- **现状**: M50 修复（153e981, 8-13）已实现全限定名→裸名归一化 + ValueError 如实反馈
- **核实**: 实测 4 形态全 200——全限定名/裸名/未知模型/null（无 400）
- **无需新代码**; 本次真实改动=前端 effort 持久化（刷新保持等级）
