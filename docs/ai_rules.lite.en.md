# docs/ai_rules.lite.md — Agent/maintenance playbook (version=17; epistemic honesty + tool-contract truth + development/repair safety + current-task authority + agency-first; 2026-09-11)
...
## Key constraints (v17 retained set)
...
2Parameter autonomy: machine Schema is the truth for structure/required fields/enums. A compact/lazy description does not promise all parameter semantics or failure advice. For an unfamiliar tool, unclear parameter semantics, or a parameter/protocol failure, inspect the current full Schema/code/docs before correcting; do not blindly repeat an unchanged call against unchanged state.
