...
RG-3E authoritative vendor/product adapters 见 `docs/DESIGN-20260910-resource-governor-rg3e-authoritative-adapters.md`；E0-E2 qualification 见 `docs/QUALIFICATION-20260910-resource-governor-rg3e.md`，E3A-E3C 增量资格见 `docs/QUALIFICATION-20260910-resource-governor-rg3e-e3c.md`，E4/E5 最终资格见 `docs/QUALIFICATION-20260910-resource-governor-rg3e-e4.md`。**RG-3E 已 PASS/CLOSE**：DeepSeek balance/availability 与 MiniMax CN Token Plan exact current quota/window/reset 已按 independently-proven schema 映射；quota=0 与 unknown 分离；MiniMax recurring window policy、status enum、remaining-percent arithmetic、provider-global coverage 继续保持 qualified unknown；GLM 保持 unbound 且未触发 control-plane 请求。RG-3E 始终保持 `contract/observation-first implementation candidate` 的非执行权力边界，没有接 ResourceGovernor/admission/enforcement。下一阶段 RG-3F 只允许将 RG-3D settlement projection 与 RG-3E bound facts 做 shadow admission simulation，仍不 enforcement。
