...
RG-3E authoritative vendor/product adapters 见 `docs/DESIGN-20260910-resource-governor-rg3e-authoritative-adapters.md`；E0-E2 qualification 见 `docs/QUALIFICATION-20260910-resource-governor-rg3e.md`，E3A-E3C 增量资格见 `docs/QUALIFICATION-20260910-resource-governor-rg3e-e3c.md`。**RG-3E 当前仍 HOLD / 未完全 CLOSE**：E0-E2 已 PASS；E3A identity discovery 已 PASS；E3B 仅对 DeepSeek standard API/account 与 MiniMax Token Plan 建立本地 non-secret binding，MiniMax region 由真实 control-plane 证明为 `cn`，tier 仍 unknown，GLM 继续 unbound；E3C 已真实只读资格化 DeepSeek `/user/balance` 与 MiniMax 中国站 `/v1/token_plan/remains` success schema。E4 typed mapping 尚未实现，provider-global coverage 仍 unknown；RG-3F 不启动，RG-3E 仍是 `contract/observation-first implementation candidate`，不接 ResourceGovernor/admission。
