...
- 2026-08-20 用户决策：**默认入口切换为 `/ui/v2`**——`/` 在产物存在时 307 重定向 `/ui/v2/`，产物缺失回退原版 index.html（旧版逐步弃用、保留兜底）

## 6. 切换方案（2026-08-20 已确认：V2 为默认入口）

| 方案 | 说明 | 回退 |
|---|---|---|
| ~~A. 保持并存（当前）~~ | 已切换（见下） | 无需 |
| **B. V2 为主入口（已实施）** | `/` 307 → `/ui/v2/`（产物存在时）；原版 static 保留作回退 | 删除重定向即回退 |
| C. 独立端口 | V2 起独立服务（如 8912） | 停进程即回退 |

2026-08-20 采用 B 变体：root 路由产物存在时 307 重定向 `/ui/v2/`，产物缺失/未构建时回退旧版 index.html（CI 场景兼容）。
