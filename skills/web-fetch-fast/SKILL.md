---
name: web-fetch-fast
description: 网页/文章链接抓取与分析的最快路径技能——用户发网页链接（今日头条/公众号/知乎/CSDN/掘金等）要求分析、摘要、展开全文时使用；目标 ≤3 次工具调用完成（实测：一次 curl + 一次解析 = <1 秒拿到正文）。核心原则：禁止盲试 web_fetch（反爬站点直接失败/截断）、禁止无目标 web_search；按探针决策树一步到位。含死链/已删文章的快速判定与止损（命中 404 特征立即停止硬试，转归档/镜像/问用户）。
---
...
## 死链/已删文章：1–2 次调用内判定，立即止损（2026-09-14 实测案例）

**案例**：`m.toutiao.com/article/7410658945672519175`——m 站壳页（~27KB，`<title>今日头条</title>` 而非文章标题）内嵌 `"type":"404"` + `"pageType":"error"`；www 版经渲染后是"抱歉，你访问的内容不存在"；`/i<ID>/info/v2/` 返回 `{"data":null,"success":false}`。三处互证：**文章已被删除/下架或 ID 有误，源站已无内容可服务**。

### 判定特征（命中任一即判死链）
1. m 站返回壳页，URL 解码后内嵌 `"type":"404"` / `"pageType":"error"`
2. Step 0 接口 HTTP 200 且 `data:null, success:false`（与限流区分：限流是 502/504 HTML，稍等重试一次即恢复；200 + `success:false` 是真死链）
3. 渲染后文案"抱歉，你访问的内容不存在"/"404错误页"

### 止损与恢复（确认死链后禁止再换 UA/换端点硬试——源站没有的东西抓不回来）
1. **立即向用户反馈死链结论**（明确区分"抓取失败"与"文章不存在"），并索要：文章标题/关键词/截图/粘贴正文
2. 拿到标题后搜转载镜像（头条文常被搬运到百家号/公众号/知乎等）
3. Wayback CDX（比 availability API 稳，后者易 429 限流）：
   `curl -s 'http://web.archive.org/cdx/search/cdx?url=toutiao.com/article/<ID>*&output=json&limit=10'`
4. 均无 → 明确告知不可恢复，结束任务
...
- ❌ **死链确认后继续换 UA/换端点硬试**：源站 404 = 没有内容，任何抓取技巧都取不回（实测多烧 6+ 次调用才收敛）；立即止损转归档/镜像/问用户
