"""Frozen A2 main-matrix qualification for semantic_execute + mechanically typed waits."""
...
def a2_gate(records: list[dict[str, Any]]) -> dict[str, Any]:
    expected_rows = 6
    ...
    wait_contract_failures = sum(
        int((row.get("worker") or {}).get("typed_wait_failure_count") or 0)
        for row in records
    )
    old_generic_wait_calls = sum(
        int((row.get("worker") or {}).get("old_generic_wait_call_count") or 0)
        for row in records
    )
    legacy_perceive_wait_misuse = sum(
        int((row.get("worker") or {}).get("legacy_perceive_wait_misuse_count") or 0)
        for row in records
    )
    snapshot_predicate_calls = sum(
        int((row.get("worker") or {}).get("snapshot_predicate_count") or 0)
        for row in records
    )

    rows_object_level_wait = sum(
        1
        for row in records
        if (row.get("worker") or {}).get("object_level_wait_invoked")
    )
    rows_action_recovery = sum(
        1
        for row in records
        if (row.get("worker") or {}).get("action_recovery_after_rejection")
    )
    rows_wait_after_rejection = sum(
        1
        for row in records
        if int((row.get("worker") or {}).get("wait_calls_after_unresolved_rejection") or 0) > 0
    )
    rows_decided_not_dispatched = sum(
        1
        for row in records
        if int((row.get("worker") or {}).get("decided_but_not_dispatched_rounds") or 0) > 0
    )
    rows_partial_only_loop = sum(
        1
        for row in records
        if int((row.get("worker") or {}).get("partial_only_loop_rounds") or 0) > 0
    )
    total_partial_checkpoints = sum(
        int((row.get("worker") or {}).get("partial_checkpoint_count") or 0)
        for row in records
    )
    max_partial_top_repeat = max(
        [int((row.get("worker") or {}).get("partial_checkpoint_top_repeat") or 0) for row in records]
        or [0]
    )
    passed = (
        infra_valid
        ...
        and mechanical_ok
        and wait_contract_failures == 0
        and old_generic_wait_calls == 0
        and legacy_perceive_wait_misuse == 0
        and snapshot_predicate_calls == 0
    ...
    return {
        ...
        "recovery_diagnostics": {
            "rows_object_level_wait_invoked": rows_object_level_wait,
            "rows_action_recovery_after_rejection": rows_action_recovery,
            "rows_wait_calls_after_unresolved_rejection": rows_wait_after_rejection,
            "rows_decided_but_not_dispatched": rows_decided_not_dispatched,
            "rows_partial_only_loop": rows_partial_only_loop,
            "total_partial_checkpoint_count": total_partial_checkpoints,
            "max_partial_checkpoint_top_repeat": max_partial_top_repeat,
        },
        ...
        "typed_wait_failures_total": wait_contract_failures,
        "typed_wait_failures_required": 0,
        "old_generic_wait_calls_total": old_generic_wait_calls,
        "old_generic_wait_calls_required": 0,
        "legacy_perceive_wait_misuse_total": legacy_perceive_wait_misuse,
        "legacy_perceive_wait_misuse_required": 0,
        "snapshot_predicate_calls_total": snapshot_predicate_calls,
        "snapshot_predicate_calls_required": 0,
        "legacy_full_action_participates_in_gate": False,
    }

# Stable runner-facing alias; this remains a treatment-only A2 main-matrix gate.
smoke_gate = a2_gate
