#!/usr/bin/env python3
"""Serial frozen MF-5.3.3 B1 direct-DOM-text + A-prime URL-role Perceive+Operate qualification runner. --max-new-rows=0 is zero-model preflight."""
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    if set(surface.get("browser_capability_names") or []) != set(BROWSER_CAPABILITIES):
        raise RuntimeError("MF-5.3.3 must expose exactly Perceive+Operate as Browser capabilities")
