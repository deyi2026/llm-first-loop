# MF-5.3.3 B1 + A-prime Perceive + Operate measured qualification protocol v0.1

Status: **FROZEN BEFORE ANY MF-5.3.3 MODEL REQUEST**
Date: 2026-09-16
Implementation anchor: `14dfdb407a098d4efa3c9c16476dd0a5239ceb39`

## Purpose

Qualify only the independently deterministic-qualified post-MF-5.3.2 B1 + A-prime treatment, using the exact same six tasks, prompts, fixture, external oracles and Hard Gate as MF-5.3.2. B1 mechanically preserves direct DOM `#text` on its exact stable parent in existing `attributes.text`; A-prime disambiguates Perceive page-URL observation as `expected_url` while Operate navigation keeps destination `url`. No tool ordering, budget, retry, grounding, completion authority or model/runtime parameter changes are part of this treatment. The model keeps ordinary task reasoning and receives only two Browser concepts:

- `browser_perceive`: seven closed root-direct branches: snapshot / exact hydrate / exact diff / four flat read-only wait kinds; `page_url` uses observation-specific `expected_url`;
