"""Frozen mechanical protocol for Semantic Operation Method OFF/ON qualification.
...
SCHEMA = "smc.browser_semantic_method_ab.v0.1"
SEED = 2026091304
...
ARMS: dict[str, dict[str, Any]] = {
    "method_off": {"method_available": False, "allowed_tools": list(ALLOWED_TOOLS)},
    "method_on": {"method_available": True, "allowed_tools": list(ALLOWED_TOOLS)},
}
...
def smoke_gate(records: list[dict[str, Any]]) -> dict[str, Any]:
    """Predeclared Method-mechanism/usability expansion gate."""
    ...
    on_rows = [row for row in smoke if row.get("arm") == "method_on"]
    off_rows = [row for row in smoke if row.get("arm") == "method_off"]
    ...
    on_hydrated = sum(bool((row.get("worker") or {}).get("method_hydrated")) for row in on_rows)
    off_hydrated = sum(bool((row.get("worker") or {}).get("method_hydrated")) for row in off_rows)
    ...
    return {
        ...
        "method_on_hydrated": on_hydrated,
        "method_on_hydrated_required": 2,
        "method_off_hydrated": off_hydrated,
        "method_off_hydrated_required": 0,
        "method_on_smc_adoption": on_smc_adopt,
        "method_off_smc_adoption": off_smc_adopt,
        "smc_adoption_required_each_arm": 2,
        "method_on_object_ok_receipts": on_object_ok,
        "method_on_object_ok_required": 1,
        "method_on_task_pass": on_task_pass,
        "method_on_task_pass_required": 1,
