#!/usr/bin/env python3
"""Serial orchestrator for the Semantic Operation Method OFF/ON A/B."""
...
def _prepare_method_seed(run_dir: Path, arm: str) -> Path:
    """Fresh seed view where target Method availability is the only content delta."""

    seed_dir = run_dir / "method-seed"
    shutil.copytree(REPO / "methods", seed_dir)
    if arm == "method_off":
        shutil.rmtree(seed_dir / METHOD_REF.removeprefix("method:"))
    return seed_dir
...
def execution_manifest(
    ...
    if surfaces["method_off"]["sha256"] != surfaces["method_on"]["sha256"]:
        raise RuntimeError("Method OFF/ON provider tool surfaces differ")
    if arm_manifests["method_off"]["method_available"]:
        raise RuntimeError("Method OFF unexpectedly exposes target Method")
    if not arm_manifests["method_on"]["method_available"]:
        raise RuntimeError("Method ON does not expose target Method")
    if arm_manifests["method_on"]["method_content_hash"] != METHOD_SHA256:
        raise RuntimeError("Method ON content hash drift")
    source_paths = [
        HERE / "protocol.py",
