#!/usr/bin/env python3
"""Offline mechanical scorer for Semantic Operation Method OFF/ON A/B."""
...
def analyze(rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    for (task_id, repeat), arms in sorted(pairs.items()):
        if set(arms) != {"method_off", "method_on"}:
            continue
        off, on = arms["method_off"], arms["method_on"]
        item: dict[str, Any] = {
            "task_id": task_id,
            "repeat": repeat,
            "method_off_pass": bool((off.get("oracle") or {}).get("pass")),
            "method_on_pass": bool((on.get("oracle") or {}).get("pass")),
            "method_on_hydrated": bool((on.get("worker") or {}).get("method_hydrated")),
        }
        for name, key in (
    ...
    arm_rows = {
        ...
        for arm in ("method_off", "method_on")
    }
    ...
    return {
        "schema": "smc.browser_semantic_method_ab.analysis.v0.1",
        "total_runs": len(rows),
        "arms": {arm: _summary(rows, arm) for arm in ("method_off", "method_on")},
        "paired": paired,
        "method": {
            arm: {
        ...
        "limitations": [
            ...
            "tool surfaces and descriptions are identical; only MethodStore target availability differs",
