#!/usr/bin/env python3
"""Serial orchestrator for Browser full-action vs semantic-execute paired smoke."""
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    for shared in ("browser_perceive", "get_tool_schema"):
        if surfaces["full_action"]["tool_sha256"].get(shared) != surfaces[
            "semantic_execute"
    ...
    if not surfaces["semantic_execute"]["semantic_usage_visible"]:
        raise RuntimeError("semantic_execute compact description lost its use path")
    full_params = set(surfaces["full_action"]["mutation_parameter_names"])
    if not {"target_id", "scope_ref", "action_id", "expected_version", "version_scope"} <= full_params:
        raise RuntimeError("full_action control no longer exposes the legacy mechanical fields")

    source_paths = [
        HERE / "protocol.py",
        HERE / "PROTOCOL.v0.4.md",
        HERE / "PLAN.v0.4.json",
    ...
    return {
        ...
        "surfaces": surfaces,
        "treatment": {
            "shared_tools_identical": True,
            "shared_tools": ["browser_perceive", "get_tool_schema"],
            "control_mutation_tool": "browser_action",
            "treatment_mutation_tool": "browser_semantic_execute",
