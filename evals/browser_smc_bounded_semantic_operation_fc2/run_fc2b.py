#!/usr/bin/env python3
"""Serial, frozen FC2-B qualification runner. max-new-rows=0 is zero-model preflight."""
...
def _model_server_fact() -> dict[str, Any]:
    ...
    if len(candidates) != 1:
        raise RuntimeError(f"expected exactly one mlx_lm.server on {MODEL_PORT}; observed={len(candidates)}")
...
def _surface_manifest(tmp_root: Path) -> dict[str, Any]:
    run_dir = tmp_root / "surface-bounded-operation"
    if run_dir.exists():
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    if dirty:
        raise RuntimeError(f"tracked working tree dirty before freeze: {dirty}")
    head = _git("rev-parse", "HEAD")
    if subprocess.run(
        ["git", "merge-base", "--is-ancestor", IMPLEMENTATION_COMMIT, head], cwd=REPO, check=False
    ).returncode != 0:
        raise RuntimeError("FC2-B implementation commit is not an ancestor of experiment commit")
    if subprocess.run(
        ["git", "diff", "--quiet", IMPLEMENTATION_COMMIT, head, "--", "src", "methods"],
        cwd=REPO,
        check=False,
    ).returncode != 0:
        raise RuntimeError("production src/methods changed after FC2-B implementation commit")

    runtime_paths = _git("ls-files", "src", "methods").splitlines()
    runtime_hashes = {path: _sha_file(REPO / path) for path in runtime_paths}
    server = _model_server_fact()
    if not server["prompt_concurrency_1"] or not server["decode_concurrency_1"]:
        raise RuntimeError("8901 is not the frozen prompt/decode concurrency=1 configuration")
    ...
    if provider_contract != expected_provider:
        raise RuntimeError(f"provider contract drift: {provider_contract}")
    ...
    if [(r["task_id"], r["repeat"]) for r in plan] != list(ROWS):
    ...
    if prompt_sha != V05_PROMPT_SHA256:
    ...
    if surface.get("operation_parameter_names") != ["clauses"]:
        raise RuntimeError("bounded-operation top-level parameter drift")
    if surface.get("operation_required") != ["clauses"] or surface.get("provider_clauses_type") != "array":
        raise RuntimeError("bounded-operation lazy provider top-level contract drift")
    if surface.get("full_operation_branch_count") != 6:
        raise RuntimeError("bounded-operation full oneOf branch count drift")
    if surface.get("full_operation_recursive_closed") is not True:
        raise RuntimeError("bounded-operation full schema is not recursively closed")
    if (
        surface.get("full_operation_max_clauses") != 8
        or surface.get("full_operation_required") != ["clauses"]
    ):
        raise RuntimeError("bounded-operation full clauses bounds/required drift")
    if surface.get("hidden_atomic_tools_present"):
        raise RuntimeError("old atomic Browser tools leaked into treatment surface")
    if not surface.get("usage_contract_visible"):
        raise RuntimeError("bounded-operation compact usage contract drift")
    ...
    return {
        ...
        "surface": surface,
        "treatment": {
            "mode": "bounded_semantic_operation_treatment_only",
            "allowed_tools": list(ALLOWED_TOOLS),
            "old_atomic_browser_tools_model_visible": False,
            "method_discovery_tool_exposed": False,
            "fixture_identity_preserved": True,
...
def run_row(row: dict[str, Any], root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    run_dir = root / f"{int(row['index']):02d}-{task_id}-r{row['repeat']}-bounded-operation"
    if run_dir.exists():
...
def main() -> int:
    ...
    if plan_path.exists():
        ...
        _atomic_json(plan_path, plan)  # type: ignore[arg-type]
    ...
    if len(existing) == len(plan):
        ...
        print(json.dumps({"qualification_complete": True, "gate_pass": gate["pass"]}, sort_keys=True))
