#!/usr/bin/env python3
"""Serial orchestrator for the Browser compact Method Card OFF/ON A/B."""
...
def _prepare_method_seed(run_dir: Path, arm: str) -> Path:
    """Fresh identical Method seed for both arms; only tool descriptions differ."""

    seed_dir = run_dir / "method-seed"
    if seed_dir.exists():
        shutil.rmtree(seed_dir)
    shutil.copytree(REPO / "methods", seed_dir)
    return seed_dir
...
def _base_env(run_dir: Path, arm: str) -> dict[str, str]:
    ...
    method_seed_dir = _prepare_method_seed(run_dir, arm)
    ...
    env.update(
        {
            ...
            "METHOD_SEED_DIR": str(method_seed_dir),
            "RUNNER_BACKGROUND": "0",
            "LFL_SMX_PERCEIVE": "",
            "MCP_SERVERS": "",
...
def _surface_manifest(arm: str, tmp_root: Path) -> dict[str, Any]:
    ...
    return {
        "surface": doc["surface"],
        "method_available": bool(doc.get("method_available")),
        "method_content_hash": doc.get("method_content_hash"),
        "method_card_expected_visible": bool(doc.get("method_card_expected_visible")),
    }
...
def execution_manifest(
    ...
    arm_manifests = {arm: _surface_manifest(arm, tmp_root) for arm in ARMS}
    surfaces = {arm: row["surface"] for arm, row in arm_manifests.items()}
    ...
    if surfaces["card_off"]["sha256"] == surfaces["card_on"]["sha256"]:
        raise RuntimeError("compact Method Card treatment did not change provider surface")
    if surfaces["card_off"]["structure_sha256"] != surfaces["card_on"]["structure_sha256"]:
        raise RuntimeError("OFF/ON surface differs outside Browser descriptions")
    if surfaces["card_off"]["method_card_visible"]:
        raise RuntimeError("card_off unexpectedly exposes compact Method Card")
    if not surfaces["card_on"]["method_card_visible"]:
        raise RuntimeError("card_on does not expose compact Method Card")
    for arm, row in arm_manifests.items():
        if not row["method_available"] or row["method_content_hash"] != METHOD_SHA256:
            raise RuntimeError(f"{arm} Method availability/hash drift")
    source_paths = [
        HERE / "protocol.py",
        ...
        HERE / "run_ab.py",
        HERE / "analyze.py",
        REPO / "methods/method-semantic-operation/METHOD.md",
    ...
    return {
        ...
        "method_availability": {
            arm: {
                "available": row["method_available"],
                "content_hash": row["method_content_hash"],
            }
            for arm, row in arm_manifests.items()
        },
        "treatment": {
            "only_browser_descriptions_differ": True,
            "structure_sha256": surfaces["card_on"]["structure_sha256"],
            "card_off_visible": surfaces["card_off"]["method_card_visible"],
            "card_on_visible": surfaces["card_on"]["method_card_visible"],
        },
