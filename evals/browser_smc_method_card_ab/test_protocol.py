...
def test_protocol_identity_and_treatment_contract() -> None:
    assert SCHEMA == "smc.browser_semantic_method_card_ab.v0.2"
    assert METHOD_REF == "method:method-semantic-operation"
    assert len(METHOD_SHA256) == 64
    assert set(ARMS) == {"card_off", "card_on"}
    assert ARMS["card_off"]["allowed_tools"] == ALLOWED_TOOLS
    assert ARMS["card_on"]["allowed_tools"] == ALLOWED_TOOLS
    assert ARMS["card_off"]["method_card_visible"] is False
    assert ARMS["card_on"]["method_card_visible"] is True
...
def _row(arm: str, *, task_pass: bool, hydrated: bool = False, object_ok: int = 0) -> dict:
    return {
        "arm": arm,
        "smoke": True,
...
def test_smoke_gate_requires_card_on_real_task_object_success() -> None:
    rows = [
        _row("card_on", task_pass=True, object_ok=1),
        _row("card_off", task_pass=False),
        _row("card_off", task_pass=False),
        _row("card_on", task_pass=False),
        _row("card_on", task_pass=False),
        _row("card_off", task_pass=False),
    ]
    ...
    for row in bad:
        if row["arm"] == "card_on":
            row["worker"]["receipt_facts"]["object_ok_count"] = 0
...
def test_method_seed_views_are_identical_and_both_contain_target_method(tmp_path: Path) -> None:
    off = _prepare_method_seed(tmp_path / "off", "card_off")
    on = _prepare_method_seed(tmp_path / "on", "card_on")
    ...
    assert target in off_ids
    assert target in on_ids
    assert off_ids == on_ids
    assert (off / target / "METHOD.md").read_bytes() == (on / target / "METHOD.md").read_bytes()
