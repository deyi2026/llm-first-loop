...
def _model_server_fact() -> dict[str, Any]:
    ...
    if not all(
        fact[key]
        for key in ("prompt_concurrency_1", "decode_concurrency_1", "max_tokens_16000")
    ):
...
def _validate_surface_pair(a: dict[str, Any], b: dict[str, Any]) -> None:
    ...
    if set((a_schema["parameters"] or {}).get("properties") or {}) != {"verb", "target_ref", "args"}:
...
def _source_hashes() -> dict[str, str]:
    paths = [
        ...
        HERE / "run_fcr.py",
        HERE / "analyze.py",
        HERE / "PROTOCOL.v0.1-FCR.md",
        HERE / "PLAN.v0.1-FCR.json",
        REPO / "docs/SMC-SEMANTIC-LOGIC-P4-AB-PROTOCOL-v0.1.json",
...
def execution_manifest(plan: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "surfaces": {
            ARM_A: {k: v for k, v in surface_a.items() if k not in {"lazy_defs", "full_defs", "wire"}},
            ARM_B: {k: v for k, v in surface_b.items() if k not in {"lazy_defs", "full_defs", "wire"}},
        },
...
def main() -> int:
    ...
    if args.preflight:
        summary = {
            ...
            "measured_row_dirs": len([p for p in root.iterdir() if p.is_dir() and p.name.startswith("row-")]),
        }
    ...
    for row in pending:
        ...
        print(json.dumps({"index": row["index"], "task": row["task_id"], "arm": row["arm"]}), flush=True)
    return 0
