#!/usr/bin/env python3
"""Serial frozen targeted delayed_wait routing reproduction diagnostic."""
...
from evals.browser_smc_delayed_wait_routing_repro_mf534r4.protocol import (  # noqa: E402
    PRODUCTION_ANCHOR,
    REF_KIND_RED_COMMIT,
    ROUTING_RED_COMMIT,
    SCHEMA,
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    for required in (ROUTING_RED_COMMIT, REF_KIND_RED_COMMIT, PRODUCTION_ANCHOR):
        if (
            subprocess.run(
    ...
    if len(plan) != 16:
        raise RuntimeError("targeted paired plan must contain 16 rows")
    ...
    return {
        ...
        "ref_kind_red_commit": REF_KIND_RED_COMMIT,
...
def main() -> int:
    ...
    if len(existing) == len(plan):
        gate = diagnostic_gate(existing)
        _atomic_json(root / "diagnostic-gate.json", gate)
        print(
            json.dumps(
                {
                    "diagnostic_complete": True,
                    "measurement_valid": gate["measurement_valid"],
                    "targeted_signal": gate["targeted_signal"],
                    "control_discriminator_observed": gate["control_discriminator_observed"],
