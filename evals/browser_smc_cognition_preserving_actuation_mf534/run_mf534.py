#!/usr/bin/env python3
"""Serial frozen MF-5.3.4 B1 direct-DOM-text + A-prime URL-role Perceive+Operate qualification runner. --max-new-rows=0 is zero-model preflight."""
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("MF-5.3.2 implementation commit is not an ancestor of experiment head")
    if (
        subprocess.run(
        ...
        raise RuntimeError("production src/methods changed after MF-5.3.2 implementation commit")
    ...
    if set(surface.get("browser_capability_names") or []) != set(BROWSER_CAPABILITIES):
        raise RuntimeError("MF-5.3.4 must expose exactly Perceive+Operate as Browser capabilities")
    ...
    expected_perceive = [
        ...
        {"action": "wait", "kind": "page_url", "fields": ["action", "expected_url", "kind", "match", "within_ms"], "required": ["action", "expected_url", "kind", "match"], "closed": True},
        {"action": "wait", "kind": "object_state", "fields": ["action", "grounding_ref", "kind", "state", "value", "within_ms"], "required": ["action", "grounding_ref", "kind", "state", "value"], "closed": True},
        {"action": "wait", "kind": "object_text", "fields": ["action", "field", "grounding_ref", "kind", "match", "text", "within_ms"], "required": ["action", "field", "grounding_ref", "kind", "match", "text"], "closed": True},
    ]
...
def run_row(row: dict[str, Any], root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    run_dir = root / f"{int(row['index']):02d}-{task_id}-r{int(row['repeat'])}-peer-capability-quality-projection"
    if run_dir.exists():
