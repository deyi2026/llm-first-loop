#!/usr/bin/env python3
"""One fresh LFL worker for frozen MF-5.3.4 B1 direct-DOM-text + A-prime URL-role Perceive+Operate qualification."""
...
def _surface(engine: Any) -> dict[str, Any]:
    ...
    operation = lazy["browser_operate"]
    full_perceive = full["browser_perceive"]
    full_operation = full["browser_operate"]
    ...
    return {
        ...
        "cognition_contract_visible": (
            ...
            and "method_ref=" not in perceive_desc
...
def _operation_message_facts(data_dir: Path, session_id: str) -> dict[str, Any]:
    ...
    for line in path.read_text(encoding="utf-8").splitlines():
        ...
        if payload.get("role") != "tool" or payload.get("tool_name") != "browser_operate":
            continue
...
def _post_operation_diagnostics(
    ...
    op_rows = [row for row in trace if row.get("name") == "browser_operate"]
