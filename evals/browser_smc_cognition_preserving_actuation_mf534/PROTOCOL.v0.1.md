# MF-5.3.4 GroundingRef object-wait measured qualification protocol v0.1

Status: **FROZEN BEFORE ANY MF-5.3.4 MODEL REQUEST**
Date: 2026-09-16
Implementation anchor: `d644e379a250f9787f51c4d16967828972a4eecd`

## Purpose

Qualify only the independently deterministic-qualified MF-5.3.4 GroundingRef object-wait contract on top of the already-qualified B1 + A-prime surface, using the exact same six tasks, prompts, fixture, external oracles and Hard Gate as MF-5.3.3. The sole new model-visible treatment is that Perceive `object_state` / `object_text` waits use the same exact `grounding_ref` field already emitted by snapshot objects and used by hydrate; legacy `object_ref` remains hidden execution compatibility only. No tool ordering, budget, retry, grounding authority, completion authority or model/runtime parameter changes are part of this treatment. The model keeps ordinary task reasoning and receives only two Browser concepts:

- `browser_perceive`: seven closed root-direct branches: snapshot / exact hydrate / exact diff / four flat read-only wait kinds; `page_url` uses observation-specific `expected_url`, and object waits use exact `grounding_ref`;
- `browser_operate`: one already-decided direct mutation; navigation alone uses destination `url`; internal historical receipt/storage schema identity remains unchanged.
...
- zero `browser_operate` failures/errors;
...
- the root-direct Perceive branch set and peer `browser_perceive`/`browser_operate` provider identity are frozen before rows run; provider lazy/full branch shapes must agree;
- model-facing Browser object projection is task-independent evidence-quality order (complete coverage -> stable/non-snapshot-local identity -> opaque id), while canonical persisted `objects_ref` evidence remains unchanged;
