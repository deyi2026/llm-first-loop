#!/usr/bin/env python3
"""Serial frozen MF-5.3 Perceive+Operate qualification runner. --max-new-rows=0 is zero-model preflight."""
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("MF-5.3 implementation commit is not an ancestor of experiment head")
    if (
        subprocess.run(
        ...
        raise RuntimeError("production src/methods changed after MF-5.3 implementation commit")
    ...
    if set(surface.get("browser_capability_names") or []) != set(BROWSER_CAPABILITIES):
        raise RuntimeError("MF-5.3 must expose exactly Perceive+Operate as Browser capabilities")
    ...
    if set(surface.get("perceive_wait_kinds") or []) != {
        ...
        raise RuntimeError(f"Perceive wait condition drift: {surface.get('perceive_wait_kinds')}")
...
def run_row(row: dict[str, Any], root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    run_dir = root / f"{int(row['index']):02d}-{task_id}-r{int(row['repeat'])}-perceive-operate"
    if run_dir.exists():
