#!/usr/bin/env python3
"""Serial orchestrator for v0.6-A read-only typed-wait FCR smoke."""
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    expected_scope = {"scope_ref", "property", "operator", "value", "timeout_ms", "interval_ms"}
    expected_object = {"object_ref", "property", "operator", "value", "timeout_ms", "interval_ms"}
    if (
        set(surface.get("scope_required") or []) != expected_scope
        or set(surface.get("object_required") or []) != expected_object
    ):
        raise RuntimeError("typed wait required-field drift")
    for key in ("scope_interval", "object_interval"):
        if surface.get(key) != {"type": "integer", "minimum": 1, "maximum": 5000}:
            raise RuntimeError(f"typed wait interval drift: {key}")
    source_paths = [
        HERE / "protocol.py",
        HERE / "PROTOCOL.v0.6-A1.md",
        HERE / "PLAN.v0.6-A1.json",
...
def run_row(row: dict[str, Any], root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    if worker_rc is None:
        ...
        status = (
            "PASS"
            if (
                worker.get("first_wait_tool") == row.get("expected_wait_tool")
                and worker.get("first_wait_structurally_complete")
                and worker.get("snapshot_before_first_wait")
                and worker.get("first_wait_result") == "satisfied"
