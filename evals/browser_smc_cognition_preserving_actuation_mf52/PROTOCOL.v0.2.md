# MF-5.2 Cognition-Preserving Browser Actuation — Ornith 6-row Qualification Protocol v0.2

Status: **FROZEN BEFORE MODEL EXECUTION**

Supersedes v0.1 for qualification execution only. v0.1 remains immutable as an invalid infrastructure attempt: rows 1-2 reached external oracle PASS but worker post-processing crashed because compact receipts were incorrectly used for boundary classification. v0.2 changes only the harness measurement path: declared/undeclared structural-boundary facts are measured exclusively from durable full operation receipts with clause verbs. Model-visible surface, task prompts, fixture bytes, Gate thresholds and execution semantics are unchanged.
