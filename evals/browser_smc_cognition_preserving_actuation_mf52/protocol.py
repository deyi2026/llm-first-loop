"""Frozen MF-5.2 cognition-preserving Browser actuation qualification protocol."""
...
def build_plan(seed: int = SEED) -> list[dict[str, Any]]:
    if seed != SEED:
        raise ValueError("MF-5.2 plan seed is frozen")
    return [
        {
            "index": index,
            "task_id": task_id,
            "repeat": repeat,
            "arm": "cognition_preserving",
...
def qualification_gate(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    first_valid = sum(
        bool((r.get("worker") or {}).get("first_operation_contract_valid")) for r in records
    )
    ...
    return {
        ...
        "first_operation_contract_valid": first_valid,
        "first_operation_contract_valid_required": expected,
        ...
        "diagnostics": {
            ...
            "operation_arg_chars": sum(
                _worker_int(r, "operation_arg_chars_total") for r in records
            ),
            "operation_result_chars": sum(
                _worker_int(r, "operation_result_chars_total") for r in records
            ),
            ...
            "single_action_calls": sum(
                _worker_int(r, "single_action_operation_count") for r in records
            ),
            "multi_action_calls": sum(
                _worker_int(r, "multi_action_operation_count") for r in records
            ),
        },
    }
