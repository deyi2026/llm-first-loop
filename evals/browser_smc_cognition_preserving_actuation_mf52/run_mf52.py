#!/usr/bin/env python3
"""Serial frozen MF-5.2 qualification runner. --max-new-rows=0 is zero-model preflight."""
...
def _surface_manifest(tmp_root: Path) -> dict[str, Any]:
    run_dir = tmp_root / "surface-cognition-preserving"
    if run_dir.exists():
...
def execution_manifest(plan: list[dict[str, Any]], tmp_root: Path) -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("MF-5.2 implementation commit is not an ancestor of experiment head")
    if (
        subprocess.run(
        ...
        raise RuntimeError("production src/methods changed after MF-5.2 implementation commit")
    ...
    if surface.get("operation_parameter_names") != ["steps"]:
        raise RuntimeError("MF-5.2 provider root must be steps")
    if surface.get("operation_required") != ["steps"]:
        raise RuntimeError("MF-5.2 provider required root drift")
    ...
    if surface.get("full_operation_recursive_closed") is not True:
        raise RuntimeError("full operation schema is not recursively closed")
    if surface.get("cognition_contract_visible") is not True:
        raise RuntimeError("cognition-preserving tool-local description drift")
    branch_do = {tuple(x) for x in surface.get("branch_do") or []}
    if ("wait",) not in branch_do or ("wait_text",) not in branch_do:
        raise RuntimeError(f"cognition-preserving wait branches missing: {branch_do}")
...
def run_row(row: dict[str, Any], root: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    run_dir = root / f"{int(row['index']):02d}-{task_id}-r{int(row['repeat'])}-cognition-preserving"
    if run_dir.exists():
            ...
            json.dumps(
                {
                    ...
                    "surface_root": manifest["surface"]["operation_parameter_names"],
                    "surface_chars": manifest["surface"]["json_chars"],
                    "branch_do": manifest["surface"]["branch_do"],
